...
for f in fileNames:
    ...
    for snr in range(EbNo):
        ...
        CSIa1Orig = rawData[f[f.find("otfs/") + 5: f.find(".mat")]][2 * snr][:20000]
        CSIb1Orig = rawData[f[f.find("otfs/") + 5: f.find(".mat")]][2 * snr + 1][:20000]

        print("bit error rate", (CSIa1Orig != CSIb1Orig).sum() / len(CSIa1Orig))

        if (CSIa1Orig != CSIb1Orig).sum() / len(CSIa1Orig) < 0.01:
            print("bit error rate is too low, continue")
...
plt.legend()
plt.savefig("match_ofdm_otfs.png", dpi=1200, bbox_inches='tight')
plt.show()
