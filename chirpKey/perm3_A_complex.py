    ...
    for addNoise in addNoises:
        for withoutSort in withoutSorts:
            ...
            for staInd in range(0, dataLenLoop, keyLenLoop):
                ...
                # 对普通的LS效果特别好
                if isComplex:
                    # print(np.allclose(np.hstack((np.real(tmpCSIb1IndPerm @ keyBin), np.imag(tmpCSIb1IndPerm @ keyBin))),
                    #                   np.hstack((np.vstack((np.real(tmpCSIb1IndPerm), np.zeros((keyLen, keyLen)))),
                    #                              np.vstack((np.zeros((keyLen, keyLen)), np.imag(tmpCSIb1IndPerm)))))
                    #                   @ np.hstack((keyBin, keyBin))))
                    tmpCSIa1IndPerm = np.hstack((np.vstack((np.real(tmpCSIa1IndPerm), np.zeros((keyLen, keyLen)))),
                                                 np.vstack((np.zeros((keyLen, keyLen)), np.imag(tmpCSIa1IndPerm)))))
                    tmpCSIb1IndPerm = np.hstack((np.vstack((np.real(tmpCSIb1IndPerm), np.zeros((keyLen, keyLen)))),
                                                 np.vstack((np.zeros((keyLen, keyLen)), np.imag(tmpCSIb1IndPerm)))))
                    tmpCSIe1IndPerm = np.hstack((np.vstack((np.real(tmpCSIe1IndPerm), np.zeros((keyLen, keyLen)))),
                                                 np.vstack((np.zeros((keyLen, keyLen)), np.imag(tmpCSIe1IndPerm)))))
                ...
                if solution == "cvxpy_perturbed_ls":
                    ...
                    a_list_number = a_list_number[0: keyLen]
                    ...
                    b_list_number = b_list_number[0: keyLen]
                    ...
                    e_list_number = e_list_number[0: keyLen]
