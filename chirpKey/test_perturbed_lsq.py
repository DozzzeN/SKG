...
# 创建问题实例
problem = pulp.LpProblem("Least_Squares", pulp.LpMinimize)
...
# 创建辅助变量
residuals = [pulp.LpVariable(f"residual_{i}", lowBound=0) for i in range(len(A_perturbed))]

# 定义目标函数
objective = pulp.lpSum(residuals)
problem += objective
