...
from segmentation.methods import find_sub_opt_segment_method_sliding
            ...
            for staInd in range(0, dataLenLoop, keyLenLoop):
                ...
                if isEncoded and bits == 4 and keyLen == 256:
                    ...
                    if isGrouped == False:
                        ...
                        # max_data_len = bch.n // 8 - bch.ecc_bits // 8  # only for bchlib.BCH(6, m=6)
                        # sixteen groups
                        # block_size = 16
                        # bch = bchlib.BCH(3, m=5)
                ...
                if solution == "stls":
                    ...
                    # 计算需要的填充次数和余数
                    rows_needed = keyLen
                    current_rows = len(tmpCSIa1IndPerm)
                    repetitions = rows_needed // current_rows  # 需要完整重复的次数
                    remainder = rows_needed % current_rows  # 剩余的行数
                    # 通过 np.tile 进行完整的行重复，并将余数部分拼接上
                    tmpCSIa1IndPerm = np.concatenate([
                        np.tile(tmpCSIa1IndPerm, (repetitions, 1)),
                        np.array(tmpCSIa1IndPerm)[:remainder, :]
                    ], axis=0)
                    tmpCSIb1IndPerm = np.concatenate([
                        np.tile(tmpCSIb1IndPerm, (repetitions, 1)),
                        np.array(tmpCSIb1IndPerm)[:remainder, :]
                    ], axis=0)
                    tmpCSIe1IndPerm = np.concatenate([
                        np.tile(tmpCSIe1IndPerm, (repetitions, 1)),
                        np.array(tmpCSIe1IndPerm)[:remainder, :]
                    ], axis=0)

                    # tmpCSIa1IndPerm = np.array(tmpCSIa1IndPerm).T
                    # tmpCSIb1IndPerm = np.array(tmpCSIb1IndPerm).T
                    # tmpCSIe1IndPerm = np.array(tmpCSIe1IndPerm).T
