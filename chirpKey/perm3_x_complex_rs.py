        ...
        for withoutSort in withoutSorts:
            ...
            for staInd in range(0, dataLenLoop, keyLenLoop):
                ...
                if isEncoded and bits == 4:
                    ...
                    # ecc = RSCodec(32)
                    # real_key_dec = np.random.randint(0, 2, keyLen)
                    # real_key = binary_to_bytes(real_key_dec)
                    # encoded = ecc.encode(real_key)
                    #
                    # keyBin = bytes_to_binary(encoded)
                    # # print("before", real_key_dec)
                    # basic = [0, 1]
                    ...
                    for i in range(int(keyLen / block_size)):
                        real_key_dec_tmp = np.random.randint(0, 2, block_size)
                        real_key_dec.extend(real_key_dec_tmp)
                        encoded = ecc.encode(binary_to_bytes(real_key_dec_tmp))
                        keyBin.extend(bytes_to_binary(encoded))
                    # print("before", len(real_key_dec), real_key_dec)
                    basic = [0, 1]
                elif isComplex:
                    if bits == 2:
                        basic = [0, 1]
                ...
                if solution == "cvxpy_perturbed_ls":
                    ...
                    if isEncoded and bits == 4:
                        ...
                        a_list_number = np.array([round(abs(i)) for i in a_list_number])
                        # a_bytes_array = binary_to_bytes(a_list_number)
                        # decoded = ecc.decode(a_bytes_array)[0]
                        # a_list_number = bytes_to_binary(decoded)
                        a_decoded = []
                        for i in range(int(keyLen / block_size)):
                            a_list_number_tmp = a_list_number[i * block_size * 2:(i + 1) * block_size * 2]
                            decoded = ecc.decode(binary_to_bytes(a_list_number_tmp))[0]
                            a_decoded.extend(bytes_to_binary(decoded))
                    ...
                    if isEncoded and bits == 4:
                        ...
                        b_list_number = np.array([round(abs(i)) for i in b_list_number])
                        # b_bytes_array = binary_to_bytes(b_list_number)
                        # # print(bytearray_diff(a_bytes_array, b_bytes_array))
                        # decoded = ecc.decode(binary_to_bytes(b_list_number))[0]
                        # b_list_number = bytes_to_binary(decoded)
                        b_decoded = []
                        for i in range(int(keyLen / block_size)):
                            b_list_number_tmp = b_list_number[i * block_size * 2:(i + 1) * block_size * 2]
                            decoded = ecc.decode(binary_to_bytes(b_list_number_tmp))[0]
                            b_decoded.extend(bytes_to_binary(decoded))
                    ...
                    if isEncoded and bits == 4:
                        ...
                        e_list_number = np.array([round(abs(i)) for i in e_list_number])
                        # 取出data部分
                        # decoded = ecc.decode(binary_to_bytes(e_list_number))[0]
                        # e_list_number = bytes_to_binary(decoded)
                        e_decoded = []
                        for i in range(int(keyLen / block_size)):
                            e_list_number_tmp = e_list_number[i * block_size * 2:(i + 1) * block_size * 2]
                            decoded = ecc.decode(binary_to_bytes(e_list_number_tmp))[0]
                            e_decoded.extend(bytes_to_binary(decoded))
