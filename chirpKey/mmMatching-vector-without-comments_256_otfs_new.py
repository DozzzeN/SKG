...
fileNames = ["../data/otfs/channelOFDM.mat",
             "../data/otfs/channelOTFS.mat"
             ]
...
for f in fileNames:
    ...
    for snr in range(10):
        print("snr:", (snr + 1) * 3)
        CSIa1Orig = rawData[f[f.find("otfs/") + 5: f.find(".mat")]][snr * 2][:2000]
        CSIb1Orig = rawData[f[f.find("otfs/") + 5: f.find(".mat")]][snr * 2 + 1][:2000]

        CSIa1Orig = np.real(CSIa1Orig)
        CSIb1Orig = np.real(CSIb1Orig)
        ...
        noise = np.random.normal(np.mean(CSIa1Orig), np.std(CSIa1Orig) / 100, size=len(CSIa1Orig))
        ...
        for staInd in range(0, len(CSIa1Orig), intvl * keyLen):
            ...
            for ii in range(permLen):
                coefLs = []
                aIndVec = np.array(
                    [aa for aa in range(permOrigInd[ii], permOrigInd[ii] + intvl, 1)])  ## for permuted CSIa1
                ...
                for jj in range(permLen, permLen * 2):
                    bIndVec = np.array(
                        [bb for bb in range(newOrigInd[jj - permLen], newOrigInd[jj - permLen] + intvl, 1)])
        ...
        mse_temp.append([snr, (CSIa1Orig - CSIb1Orig) @ (CSIa1Orig - CSIb1Orig)])
...
plt.legend()
plt.savefig("match_ofdm_otfs_new.png", dpi=1200, bbox_inches='tight')
plt.show()

plt.figure()
for i in range(len(mse)):
    plt.plot([x[0] for x in mse[i]], [x[1] for x in mse[i]], label=fileNames[i]
    [fileNames[i].find("otfs/") + 5: fileNames[i].find(".mat")])
plt.legend()
plt.show()
