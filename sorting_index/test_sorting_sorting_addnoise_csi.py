...
CSIa1Orig = rawDataA['CSIa'][0]
CSIb1Orig = rawDataB['CSIb'][0]
...
addNoise = "mul"
# reader = csv.reader(open("64/mi_seed.csv"))
# seeds = []
# for line in reader:
#     seeds.append(line)
...
for staInd in range(0, dataLen, int(keyLen / 10)):
    ...
    while True:
        ...
        if addNoise == "add":
            noiseOrig = np.random.uniform(0, np.std(CSIa1Orig) * 3.2, size=len(CSIa1Orig))
            ...
            randomMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 5, size=(keyLen, keyLen))
