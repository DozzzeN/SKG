...
for k in range(2, 10):
    ...
    # 难以找到上界
    print("simulate", np.power(simulates / times, n))
    p = (3 * np.square(a - b) - 2 * (a - b + 3 * ma - 3 * mb) * (a - b - ma + mb)) / (6 * np.square(ma - mb))
    print("probability", np.power(p, 2 * n - 2))
    print("probability", np.power(p, 2 * n - 2) / np.power(2, n - 2))
    print("probability", np.power(p, n))
