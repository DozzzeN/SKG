    ...
    for reuse in range(reuseTimes):
        ...
        if addNoise == "mul":
            ...
            if singular:
                # 使用奇异矩阵
                if deRank == 2:
                    randomMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen - 2))
                    linearElement = []
                    randomCoff = np.random.uniform(0, np.std(CSIa1Orig) * 4, size=keyLen - 2)
                    ...
                    # print(len(randomMatrix), len(randomMatrix[0]))
                    # print(np.linalg.matrix_rank(randomMatrix))
                    # print(np.linalg.pinv(randomMatrix))
                    # print(np.linalg.inv(randomMatrix))  # bug：依然可以求逆矩阵，但和广义逆矩阵差别很大
                elif deRank == 1:
            ...
            # 两者相等说明有相容解
            publicMatrix = randomMatrix.copy()
            # print(tmpCSIa1)
            # print(np.matmul(np.matmul(tmpCSIa1, np.linalg.pinv(randomMatrix)), randomMatrix))
            # print("the real solution", tmpCSIa1b)
            # print(min(CSIa1Orig), max(CSIa1Orig))
            # print(min(tmpCSIa1b), max(tmpCSIa1b))
            particularSolution = np.matmul(tmpCSIa1, np.linalg.pinv(randomMatrix))
            # print("particular solution", particularSolution)
            generalSolution1 = particularSolution + np.matmul(np.random.normal(0, np.std(CSIa1Orig) * 4, keyLen),
                                                              np.identity(keyLen) - np.matmul(randomMatrix,
                                                                                              np.linalg.pinv(
                                                                                                  randomMatrix)))
            # with higher variance
            # generalSolution2 = particularSolution + np.matmul(np.ones(keyLen) * 1000 *
            #     - abs(min(min(CSIa1Orig), min(CSIb1Orig)) - min(particularSolution)),
            #     np.identity(keyLen) - np.matmul(randomMatrix, np.linalg.pinv(randomMatrix)))
            # generalSolution2 = particularSolution + np.matmul(np.ones(keyLen) * 1000 *
            #     abs(max(max(CSIa1Orig), max(CSIb1Orig)) - max(particularSolution)),
            #     np.identity(keyLen) - np.matmul(randomMatrix, np.linalg.pinv(randomMatrix)))
            generalSolution2 = particularSolution + np.matmul(np.random.uniform(
                keyLen / segLen * -abs(min(min(CSIa1Orig), min(CSIb1Orig)) - min(particularSolution)),
                keyLen / segLen * abs(max(max(CSIa1Orig), max(CSIb1Orig)) - max(particularSolution)), keyLen),
                np.identity(keyLen) - np.matmul(randomMatrix, np.linalg.pinv(randomMatrix)))
            # print("general solution1", generalSolution1)
            # print(min(generalSolution1), max(generalSolution1))
            # print("general solution2", generalSolution2)
            # print(min(generalSolution2), max(generalSolution2))
            ...
            # print("general solution1", np.matmul(generalSolution1, randomMatrix))
            # print("general solution1", np.matmul(generalSolution1, randomMatrix))
            ...
            # print("particular solution", np.matmul(particularSolution, randomMatrix))
            # print("general solution1", np.matmul(generalSolution1, randomMatrix))
            # print("general solution2", np.matmul(generalSolution2, randomMatrix))
        ...
        if reuse == 0:
            ...
            if singular:
                ...
                beforeMulCSIe2 = np.matmul(np.array(beforeSortCSIe2), np.linalg.pinv(publicMatrix)) + \
                                 np.matmul(np.random.normal(0, np.std(CSIa1Orig) * 10, keyLen),
                                           np.identity(keyLen) -
