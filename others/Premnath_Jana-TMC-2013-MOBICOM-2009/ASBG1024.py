...
# mi1 0.572162941   0.0 0.6613609436975223 0.3784062226343172
# si1 0.5399636513  0.0 0.7500597014925373 0.4050049751243781
# mo1 0.5916329926  0.0 0.6237212750185323 0.36901408450704226
# so1 0.644046169   0.0 0.7685552534212989 0.49498506664289216
# average BMR: 1-(0.572162941 +0.5399636513+0.5916329926+0.644046169)/4=0.4130485615249999
# average BGR: (0.3784062226343172+0.4050049751243781+0.36901408450704226+0.49498506664289216)/4=0.4118525872271574
...
for staInd in range(0, int(dataLen), keyLen):
    ...
    tmpCSIn1 = np.random.normal(loc=np.mean(tmpCSIa1), scale=np.std(tmpCSIa1, ddof=1), size=len(tmpCSIa1))
    ...
    print("\033[0;32;40ma-b", sum2, 0 if sum1 == 0 else sum2 / sum1, "\033[0m")
    print("a-e1", sum31, 0 if sum1 == 0 else sum31 / sum1)
    print("a-e2", sum32, 0 if sum1 == 0 else sum32 / sum1)
    print("a-n1", sum41, 0 if sum1 == 0 else sum41 / sum1)
    originSum += sum1
    correctSum += sum2
    randomSum1 += sum31
    randomSum2 += sum32
