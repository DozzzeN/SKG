...
def listToHilbertCurveIndexWithFixedParams(list, p, grid_size):
    ...
    print(findMinInterval(list) / 2 * math.sqrt(2))
    total_grids = findMaxX(points) * findMaxY(points)
    pp = int(math.log10(total_grids) / math.log10(2))
    print(total_grids, pp)
...
for staInd in range(0, len(CSIa1Orig), intvl * keyLen):
    ...
    # # 转为二进制
    # for i in range(len(a_list_number)):
    #     a_list += bin(a_list_number[i])[2:]
    # for i in range(len(b_list_number)):
    #     b_list += bin(b_list_number[i])[2:]
    # for i in range(len(e_list_number)):
    #     e_list += bin(e_list_number[i])[2:]
    # for i in range(len(n_list_number)):
    #     n_list += bin(n_list_number[i])[2:]
    #
    # # 对齐密钥，随机补全
