...
from segmentation.methods import find_opt_segment_method, segment_sequence, compute_min_dtw, \
    ...
    search_random_matrix_uniform, insert_random_numbers, modify_segments
        ...
        for withoutSort in withoutSorts:
            ...
            while staInd < dataLenLoop:
                ...
                if withIndexValue:
                    ...
                    tmpCSIb1 = (tmpCSIb1 - np.min(tmpCSIb1)) / (np.max(tmpCSIb1) - np.min(tmpCSIb1))
                    tmpCSIe1 = (tmpCSIe1 - np.min(tmpCSIe1)) / (np.max(tmpCSIe1) - np.min(tmpCSIe1))
                    tmpCSIa1Index = (tmpCSIa1Index - np.min(tmpCSIa1Index)) / (
                                np.max(tmpCSIa1Index) - np.min(tmpCSIa1Index))
                    tmpCSIb1Index = (tmpCSIb1Index - np.min(tmpCSIb1Index)) / (
                                np.max(tmpCSIb1Index) - np.min(tmpCSIb1Index))
                    tmpCSIe1Index = (tmpCSIe1Index - np.min(tmpCSIe1Index)) / (
                                np.max(tmpCSIe1Index) - np.min(tmpCSIe1Index))
                ...
                if segment_option == "find_search" or segment_option == "find_search_pair":
                    ...
                    if findError is False:
                        ...
                        print("segment types", len(segment_length_A))
                        for i in range(len(segment_length_A)):
                            # 加入额外锚点数据
                            tmpCSIa1IndReshape = segment_sequence(tmpCSIa1Ind,
                                                                  np.array(segment_length_A[i]).astype(int))
                            tmpCSIb1IndReshape = segment_sequence(tmpCSIb1Ind,
                                                                  np.array(segment_length_A[i]).astype(int))
                            tmpCSIe1IndReshape = segment_sequence(tmpCSIe1Ind,
                                                                  np.array(segment_length_A[i]).astype(int))
                            tmpCSIa1BckReshape = segment_sequence(tmpCSIa1Bck,
                                                                  np.array(segment_length_A[i]).astype(int))

                            if i == 0:
                                # 只更新一次
                                remain_len = keyLen + segNum - sum(np.array(segment_length_A[i]).astype(int))
                                keyLen += remain_len
                            tmpCSIa1IndReshape, modifications = modify_segments(tmpCSIa1IndReshape, remain_len,
                                                                                staInd + 100000)

                            tmpCSIb1IndReshape = [list(segment) for segment in tmpCSIb1IndReshape]
                            tmpCSIe1IndReshape = [list(segment) for segment in tmpCSIe1IndReshape]
                            tmpCSIa1BckReshape = [list(segment) for segment in tmpCSIa1BckReshape]
                            for j, position, random_number in modifications:
                                tmpCSIa1BckReshape[j].insert(position, random_number)
                                tmpCSIb1IndReshape[j].insert(position, random_number)
                                tmpCSIe1IndReshape[j].insert(position, random_number)
                                segment_length_A[i][j] += 1

                                for k in range(len(segment_length_B)):
                                    if np.array_equal(segment_length_B[k], segment_length_A[i]):
                                        segment_length_B[k][j] += 1
                                        break
                                for k in range(len(segment_length_E)):
                                    if np.array_equal(segment_length_E[k], segment_length_A[i]):
                                        segment_length_E[k][j] += 1
                                        break

                            # before_add_mean_pearson_ab.append(abs(pearsonr(np.hstack(np.vstack(tmpCSIa1IndReshape)),
                            #                                                np.hstack(np.vstack(tmpCSIb1IndReshape)))[0]))
                            # before_add_mean_pearson_ae.append(abs(pearsonr(np.hstack(np.vstack(tmpCSIa1IndReshape)),
                            #                                                np.hstack(np.vstack(tmpCSIe1IndReshape)))[0]))

                            if withIndexValue:
                                tmpCSIa1Inds.append(np.vstack(tmpCSIa1IndReshape))
                                tmpCSIb1Inds.append(np.vstack(tmpCSIb1IndReshape))
                                tmpCSIe1Inds.append(np.vstack(tmpCSIe1IndReshape))
                                tmpCSIa1Bcks.append(np.vstack(tmpCSIa1BckReshape))
                            else:
                                tmpCSIa1Inds.append(np.hstack((tmpCSIa1IndReshape)))
                                tmpCSIb1Inds.append(np.hstack((tmpCSIb1IndReshape)))
                                tmpCSIe1Inds.append(np.hstack((tmpCSIe1IndReshape)))
                                tmpCSIa1Bcks.append(np.hstack((tmpCSIa1BckReshape)))

                            # after_add_mean_pearson_ab.append(
                            #     abs(pearsonr(np.hstack(tmpCSIa1Inds[i]), np.hstack(tmpCSIb1Inds[i]))[0]))
                            # after_add_mean_pearson_ae.append(
                            #     abs(pearsonr(np.hstack(tmpCSIa1Inds[i]), np.hstack(tmpCSIe1Inds[i]))[0]))
                        ...
                        for i in range(len(segment_length_A)):
                            ...
                            a_list_number_tmp, dists = search_index_with_segment(
                                tmpCSIa1Bcks[i], tmpCSIa1Inds[i], np.array(segment_length_A[i]).astype(int))
                        ...
                        for i in range(len(segment_length_B)):
                            ...
                            #     continue
                            for j in range(len(segment_length_A)):
                                if np.array_equal(segment_length_A[j], segment_length_B[i]):
                                    tmpCSIa1Ind = tmpCSIa1Inds[j]
                        ...
                        for i in range(len(segment_length_E)):
                            ...
                            #     continue
                            for j in range(len(segment_length_A)):
                                if np.array_equal(segment_length_A[j], segment_length_E[i]):
                                    tmpCSIa1Ind = tmpCSIa1Inds[j]
