...
#             min_dtw = min(min_dtw, dtw_metric(data[i], data[j])[0])
        ...
        for withoutSort in withoutSorts:
            ...
            for staInd in range(0, dataLenLoop, keyLenLoop):
                ...
                if segment_option == "opt_pair":
                    ...
                    start_time = time.time_ns()
                    segment_method_A = find_sub_opt_segment_method_down(tmpCSIa1Ind, tmpCSIa1Ind, 3, int(keyLen / segLen))
                    segment_method_B = find_sub_opt_segment_method_down(tmpCSIb1Ind, tmpCSIb1Ind, 3, int(keyLen / segLen))
                    ...
                    segment_method_A_single = find_sub_opt_segment_method_down(tmpCSIa1Ind, tmpCSIa1Ind, 3, int(keyLen / segLen))
                    segment_method_B_single = find_sub_opt_segment_method_down(tmpCSIb1Ind, tmpCSIb1Ind, 3, int(keyLen / segLen))

                    start_time = time.time_ns()
                    segment_method_A = find_sub_opt_segment_method_down(tmpCSIa1Ind, simCSIa1Ind, 3, int(keyLen / segLen))
                    segment_method_B = find_sub_opt_segment_method_down(tmpCSIb1Ind, simCSIb1Ind, 3, int(keyLen / segLen))
                    if len(segment_method_A) != int(keyLen / segLen) or len(segment_method_B) != int(keyLen / segLen):
                        print("未找到各自的次优解")
                        continue
                    print("staInd", staInd, "segment_method", segment_method_A, segment_method_B)
                    ...
                    segment_method_ori_single = find_opt_segment_method(tmpCSIa1Ind, tmpCSIa1Ind, 3, int(keyLen / segLen))
                    ...
                    # 次优分段（是否固定分段个数）
                    # segment_method_ori = find_sub_opt_segment_method_down(tmpCSIa1Ind, tmpCSIb1Ind, 3, int(keyLen / segLen))
                    ...
                    # 次优分段（是否固定分段个数）
                    segment_method_ori = find_sub_opt_segment_method_down(tmpCSIa1Ind, tmpCSIa1Ind, 3, int(keyLen / segLen))
