            ...
            while staInd < dataLenLoop:
                ...
                if segment_option == "opt_pair":
                    ...
                    print("sum dtw of ab / aa / bb",
                          np.sum(compute_all_dtw(tmpCSIa1IndReshape, tmpCSIb1IndReshape)),
                          np.sum(compute_all_dtw(tmpCSIa1IndReshape, tmpCSIa1IndReshape)),
                          np.sum(compute_all_dtw(tmpCSIb1IndReshape, tmpCSIb1IndReshape)))

                    # 添加额外数据
                    remain_len = keyLen + segNum - sum(segment_method_ori)
                    # remain_len = keyLen + segLen - sum(segment_method_ori)
                    # tmpCSIa1IndReshape, modifications = modify_segments_position_compared_with_one_segments(
                    #     tmpCSIa1IndReshape, remain_len, staInd + 100000)
                    tmpCSIa1IndReshape, modifications = modify_segments_with_even_addition(
                        tmpCSIa1IndReshape, remain_len, keyLen + segNum, staInd + 100000)
                    # tmpCSIa1IndReshape, modifications = modify_segments_with_even_addition(
                    #     tmpCSIa1IndReshape, remain_len, keyLen + segLen, staInd + 100000)
                    keyLen = sum(segment_method_ori) + remain_len

                    segment_method_ori = [len(segment) for segment in tmpCSIa1IndReshape]
                    ...
                    for i, position, random_number in modifications:
                        tmpCSIb1IndReshape[i].insert(position, random_number)
                        tmpCSIe1IndReshape[i].insert(position, random_number)
