...
# 三种方法自上而下分段，找出分段方法中的最优解：分段之间平均DTW距离最大
np.random.seed(0)
data = np.random.permutation(3 * 8)
min_length = 2
max_length = 7
num_segments = 8
########################################################################################
...
for i in range(len(each_layer_optimal)):
    ...
    for segment_method in each_layer_optimal[i]:
        ...
        print(segment_method, compute_mean_dtw(segment))
...
for data_seg in opt_data_seg:
    ...
    print(data_seg, compute_mean_dtw(segment_sequence(data, data_seg)))
