...
from segmentation.methods import find_opt_segment_method, segment_sequence, compute_min_dtw, \
    ...
    replace_segments_position_compared_with_all_segments_no_limit_pair, entropyPerm, \
    replace_segments_position_compared_with_all_segments_limited_factor

from segmentation.optimization.test_gen_matrix import generate_matrix_and_solve_diag
import numpy as np
            ...
            for withoutSort in withoutSorts:
                ...
                #     staInd += keyLenLoop
                    ...
                    if isPartPerm:
                        ...
                        # extCSIb1 = []
                        # extCSIe1 = []
                        # for ext in range(segNum * 100):
                        #     np.random.seed(100000 + ext)
                        #     shuffleInd = np.random.permutation(keyLen)
                        #     extCSIa1.extend(tmpCSIa1[shuffleInd])
                        #     extCSIb1.extend(tmpCSIb1[shuffleInd])
                        #     extCSIe1.extend(tmpCSIe1[shuffleInd])
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 4, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)[:keyLen]
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)[:keyLen]
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)[:keyLen]
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # print(pearsonr(tmpCSIa1, tmpCSIb1)[0])
                        ...
                        # extCSIe1 = []
                        # np.random.seed(100000)
                        # dummy = np.random.uniform(np.min(tmpCSIa1), np.max(tmpCSIa1), keyLen)
                        # for ext in range(segNum * 100):
                        #     np.random.seed(100000 + ext)
                        #     shuffleInd = np.random.permutation(keyLen)
                        #     extCSIa1.extend(dummy[shuffleInd])
                        #     extCSIb1.extend(dummy[shuffleInd])
                        #     extCSIe1.extend(dummy[shuffleInd])
                        #     extCSIa1.extend(tmpCSIa1[shuffleInd])
                        #     extCSIb1.extend(tmpCSIb1[shuffleInd])
                        #     extCSIe1.extend(tmpCSIe1[shuffleInd])
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 4, size=(len(extCSIa1), len(extCSIa1)))
                        #
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # tmpCSIa1 = np.array(tmpCSIa1).reshape(keyLen, segNum * 100 * 2)
                        # tmpCSIb1 = np.array(tmpCSIb1).reshape(keyLen, segNum * 100 * 2)
                        # tmpCSIe1 = np.array(tmpCSIe1).reshape(keyLen, segNum * 100 * 2)
                        # tmpCSIa1 = np.mean(tmpCSIa1, axis=1)
                        # tmpCSIb1 = np.mean(tmpCSIb1, axis=1)
                        # tmpCSIe1 = np.mean(tmpCSIe1, axis=1)
                        # print(pearsonr(tmpCSIa1, tmpCSIb1)[0])

                        # 循环矩阵扩展 - 乘矩阵后求均值+noise
                        # 1.0
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # if isExtraPrint:
                        #     print(pearsonr(tmpCSIa1, tmpCSIb1)[0])
                        # extCSIa1 = circulant(tmpCSIa1[::-1])
                        # extCSIb1 = circulant(tmpCSIb1[::-1])
                        # extCSIe1 = circulant(tmpCSIe1[::-1])
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        ...
                        # tmpCSIb1 = np.mean(tmpCSIb1, axis=1)
                        # tmpCSIe1 = np.mean(tmpCSIe1, axis=1)
                        # randomMatrix2 = np.random.uniform(0, 0.01, size=keyLen)
                        # tmpCSIa1 = tmpCSIa1 + randomMatrix2
                        # tmpCSIb1 = tmpCSIb1 + randomMatrix2
                        # tmpCSIe1 = tmpCSIe1 + randomMatrix2
                        # if isExtraPrint:
                        #     print(pearsonr(tmpCSIa1, tmpCSIb1)[0])

                        # 循环矩阵扩展 - 乘矩阵后求均值 (baseline)
                        # 0.997544506
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        ...
                        # extCSIe1 = circulant(tmpCSIe1[::-1])
                        # # 效果更差
                        # # extCSIa1 = toeplitz(tmpCSIa1[::-1])
                        # # extCSIb1 = toeplitz(tmpCSIb1[::-1])
                        # # extCSIe1 = toeplitz(tmpCSIe1[::-1])
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)
                        ...
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # # tmpCSIa1 = np.matmul(tmpCSIa1, randomMatrix)[:keyLen]
                        # # tmpCSIb1 = np.matmul(tmpCSIb1, randomMatrix)[:keyLen]
                        # # tmpCSIe1 = np.matmul(tmpCSIe1, randomMatrix)[:keyLen]
                        # # 按行
                        # tmpCSIa1 = np.mean(tmpCSIa1, axis=1)
                        # tmpCSIb1 = np.mean(tmpCSIb1, axis=1)
                        # tmpCSIe1 = np.mean(tmpCSIe1, axis=1)
                        # # 按列
                        # # tmpCSIa1 = np.mean(tmpCSIa1, axis=0)
                        # # tmpCSIb1 = np.mean(tmpCSIb1, axis=0)
                        # # tmpCSIe1 = np.mean(tmpCSIe1, axis=0)
                        # if isExtraPrint:
                        #     print(pearsonr(tmpCSIa1, tmpCSIb1)[0])
                        ...
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)
                        # randomMatrix2 = np.random.uniform(0, 1, size=len(extCSIa1))
                        # tmpCSIa1 = np.matmul(tmpCSIa1, randomMatrix2)
                        # tmpCSIb1 = np.matmul(tmpCSIb1, randomMatrix2)
                        # tmpCSIe1 = np.matmul(tmpCSIe1, randomMatrix2)
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # if isExtraPrint:
                        ...
                        # # 循环矩阵扩展 - 先置换，后乘一个大矩阵，求均值
                        # # 0.998620119 / 0.604801987
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # if isExtraPrint:
                        ...
                        # extCSIb1 = []
                        # extCSIe1 = []
                        # for ext in range(segNum):
                        #     np.random.seed(100000 + ext)
                        #     shuffleInd = np.random.permutation(keyLen)
                        #     extCSIa1.extend(tmpCSIa1[shuffleInd])
                        #     extCSIb1.extend(tmpCSIb1[shuffleInd])
                        #     extCSIe1.extend(tmpCSIe1[shuffleInd])
                        # extCSIa1 = circulant(extCSIa1[::-1])
                        # extCSIb1 = circulant(extCSIb1[::-1])
                        # extCSIe1 = circulant(extCSIe1[::-1])
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)[:keyLen]
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)[:keyLen]
                        ...
                        # tmpCSIe1 = np.mean(tmpCSIe1, axis=1)
                        # # np.random.seed(10000)
                        # # randomMatrix2 = np.random.uniform(0, 0.001, size=keyLen)
                        # # tmpCSIa1 = tmpCSIa1 + randomMatrix2
                        # # tmpCSIb1 = tmpCSIb1 + randomMatrix2
                        # # tmpCSIe1 = tmpCSIe1 + randomMatrix2
                        # if isExtraPrint:
                        ...
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)[:keyLen].T[0]
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)[:keyLen].T[0]
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)[:keyLen].T[0]
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # # np.random.seed(10000)
                        ...
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)[:keyLen].T
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)[:keyLen].T
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)[:keyLen].T
                        # max_corr = 0
                        # max_corr_index = -1
                        # for i in range(len(tmpCSIa1)):
                        #     corr = pearsonr(tmpCSIa1[i], tmpCSIb1[i])[0]
                        #     if corr > max_corr:
                        #         max_corr = corr
                        #         max_corr_index = i
                        # tmpCSIa1 = tmpCSIa1[max_corr_index]
                        # tmpCSIb1 = tmpCSIb1[max_corr_index]
                        # tmpCSIe1 = tmpCSIe1[max_corr_index]
                        # # 处理后不进行均值化
                        # # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        ...
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)[0]
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)[0]
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)[0]
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                        # tmpCSIb1 = tmpCSIb1 - np.mean(tmpCSIb1)
                        # tmpCSIe1 = tmpCSIe1 - np.mean(tmpCSIe1)
                        # if isExtraPrint:
                        ...
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix).T
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix).T
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix).T
                        # max_corr = 0
                        # max_corr_index = -1
                        # for i in range(len(tmpCSIa1)):
                        #     corr = pearsonr(tmpCSIa1[i], tmpCSIb1[i])[0]
                        ...
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        # mulCSIa1 = np.matmul(extCSIa1, randomMatrix)
                        # mulCSIb1 = np.matmul(extCSIb1, randomMatrix)
                        # mulCSIe1 = np.matmul(extCSIe1, randomMatrix)
                        # max_corr = 0
                        # for i in range(len(mulCSIa1)):
                        #     corr = pearsonr(mulCSIa1[i], mulCSIb1[i])[0]
                        #     if corr > max_corr:
                        #         max_corr = corr
                        #         tmpCSIa1 = mulCSIa1[i]
                        #         tmpCSIb1 = mulCSIb1[i]
                        ...
                        # extCSIe1 = circulant(tmpCSIe1[::-1])
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), 4 * len(extCSIa1)))
                        # tmpCSIa1 = np.matmul(extCSIa1, randomMatrix)
                        # tmpCSIb1 = np.matmul(extCSIb1, randomMatrix)
                        # tmpCSIe1 = np.matmul(extCSIe1, randomMatrix)
                        # tmpCSIa1 = tmpCSIa1 - np.mean(tmpCSIa1)
                    ...
                    if segment_option == "find_search":
                        ...
                        # 针对 循环矩阵扩展 - 随机选一行
                        # 下列的tmpCSIa1Ind == tmpCSIa1
                        # tmpCSIa1Ind = extCSIa1[max_corr_index] @ randomMatrix
                        # tmpCSIb1Ind = extCSIb1[max_corr_index] @ randomMatrix
                        # np.random.seed(0)
                        # tmpCSIe1Ind = np.random.normal(np.mean(extCSIa1[max_corr_index]), np.std(extCSIa1[max_corr_index]),
                        #                                extCSIa1[max_corr_index].shape) @ randomMatrix

                        # tmpCSIa1Ind = tmpCSIa1
                        # tmpCSIb1Ind = tmpCSIb1
                        # tmpCSIe1Ind = tmpCSIe1

                        # 针对 循环矩阵扩展 - 先置换，后乘一个大矩阵，任取一行
                        # tmpCSIa1Ind = np.matmul(extCSIa1, randomMatrix)[: keyLen].T[max_corr_index]
                        # tmpCSIb1Ind = np.matmul(extCSIb1, randomMatrix)[: keyLen].T[max_corr_index]
                        # np.random.seed(0)
                        # tmpCSIe1Ind = np.matmul(np.random.normal(np.mean(extCSIa1), np.std(extCSIa1),
                        #                                extCSIa1.shape), randomMatrix)[: keyLen].T[max_corr_index]
                        # 上述操作等价于左乘切片矩阵，右乘向量
                        leftMatrix = np.zeros((keyLen, len(extCSIa1)))
                        for i in range(keyLen):
                            leftMatrix[i, i] = 1
                        rightMatrix = np.zeros(len(extCSIa1))
                        rightMatrix[max_corr_index] = 1
                        tmpCSIa1Ind = leftMatrix @ (extCSIa1 @ randomMatrix) @ rightMatrix
                        tmpCSIb1Ind = leftMatrix @ (extCSIb1 @ randomMatrix) @ rightMatrix
                        np.random.seed(0)
                        tmpCSIe1Ind = leftMatrix @ (np.random.normal(np.mean(extCSIa1), np.std(extCSIa1),
                                                       extCSIa1.shape) @ randomMatrix) @ rightMatrix

                        # 均值化对安全性影响不大，但是可以提升准确率
                        # tmpCSIa1Ind = tmpCSIa1Ind - np.mean(tmpCSIa1Ind)
                        # tmpCSIb1Ind = tmpCSIb1Ind - np.mean(tmpCSIb1Ind)
                        # tmpCSIe1Ind = tmpCSIe1Ind - np.mean(tmpCSIe1Ind)
                        ...
                        # 考虑缩放因子的大小，过大或者过小的缩放因子会导致B的数据变差
                        # tmpCSIa1IndReshape, modifications = replace_segments_position_compared_with_all_segments_limited_factor(
                        #     tmpCSIa1IndReshape, replace_number, staInd + 100000)
                        ...
                        # 替换方法：随机矩阵
                        # transformMatrix = np.linalg.lstsq(tmpCSIa1Ind.reshape(1, -1), tmpCSIa1IndTmp.reshape(1, -1), rcond=None)[0]
                        ...
                        # 不同矩阵维度的转换
                        transformMatrix = np.linalg.lstsq(np.array(np.hstack(tmpCSIa1Ind)).reshape(segNum, segLen),
                                                          np.array(np.hstack(tmpCSIa1IndTmp)).reshape(segNum, segLen), rcond=None)[0]
                        # if isExtraPrint:
                        #     print(transformMatrix.shape)
                        tmpCSIa1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIa1Ind).reshape(segNum, segLen).T).flatten()
                        tmpCSIb1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIb1Ind).reshape(segNum, segLen).T).flatten()
                        tmpCSIe1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIe1Ind).reshape(segNum, segLen).T).flatten()

                        # publicMatrix = np.linalg.lstsq(np.array(extCSIa1[max_corr_index]).reshape(1, -1),
                        #                                tmpCSIa1IndTmp.reshape(1, -1), rcond=None)[0]
                        # tmpCSIa1IndTmp = np.matmul(np.array(extCSIa1[max_corr_index]).T, publicMatrix).T
                        # tmpCSIb1IndTmp = np.matmul(extCSIb1[max_corr_index], publicMatrix).T
                        # np.random.seed(0)
                        # tmpCSIe1IndTmp = np.matmul(np.random.normal(np.mean(extCSIa1[max_corr_index]),
                        #                                np.std(extCSIa1[max_corr_index]),
                        #                                extCSIa1[max_corr_index].shape).T, publicMatrix).T

                        # 替换方法：随机向量
                        # multiplicative diag matrix
                        # transformMatrix = np.diag(np.array(np.hstack(tmpCSIa1IndTmp)) / (np.array(np.hstack(tmpCSIa1Ind))))
                        # tmpCSIa1IndTmp = transformMatrix @ np.hstack(tmpCSIa1Ind)
                        # tmpCSIb1IndTmp = transformMatrix @ np.hstack(tmpCSIb1Ind)
                        # tmpCSIe1IndTmp = transformMatrix @ np.hstack(tmpCSIe1Ind)

                        # additive diag matrix
                        # transformMatrix = tmpCSIa1IndTmp - tmpCSIa1Ind
                        # tmpCSIa1IndTmp = tmpCSIa1Ind + transformMatrix
                        # tmpCSIb1IndTmp = tmpCSIb1Ind + transformMatrix
                        # tmpCSIe1IndTmp = tmpCSIe1Ind + transformMatrix
                        # publicMatrix = np.linalg.lstsq(np.array(extCSIa1[max_corr_index]).reshape(1, -1),
                        #                                tmpCSIa1IndTmp.reshape(1, -1), rcond=None)[0]
                        # 方法1：加性向量等价于 (0.994743166)
                        # tmpCSIa1IndTmp = A * randomMatrix + transformMatrix = A * otherRandomMatrix + offsetMatrix
                        # A := extCSIa1[max_corr_index])
                        # np.random.seed(staInd)
                        # otherRandomMatrix = np.random.uniform(0, 1, size=(keyLen, keyLen))
                        # offsetMatrix = tmpCSIa1IndTmp - extCSIa1[max_corr_index][:keyLen] @ otherRandomMatrix
                        # tmpCSIa1Raw = extCSIa1[max_corr_index] @ otherRandomMatrix + offsetMatrix
                        # publicMatrix = offsetMatrix

                        # 针对 循环矩阵扩展 - 先置换，后乘一个大矩阵，任取一行
                        # tmpCSIa1IndTmp = A + transformMatrix = A' + offsetMatrix
                        # A := leftMatrix @ (extCSIa1 @ randomMatrix) @ rightMatrix
                        # A' := leftMatrix @ (extCSIa1 @ otherRandomMatrix) @ rightMatrix
                        np.random.seed(staInd)
                        otherRandomMatrix = np.random.uniform(0, 1, size=(len(extCSIa1), len(extCSIa1)))
                        offsetMatrix = tmpCSIa1IndTmp - leftMatrix @ (extCSIa1 @ otherRandomMatrix) @ rightMatrix
                        publicMatrix = offsetMatrix

                        # 不同维度的矩阵时 等价的公开矩阵
                        # 方法：扩展原矩阵T，形成一个类似对角矩阵：
                        # [T          ]
                        # [   T       ]
                        # [      T    ]
                        # [          T]
                        # transformMatrix = scipy.sparse.kron(scipy.sparse.eye(segNum), transformMatrix).toarray()
                        # 等价的公开矩阵
                        # publicMatrix = randomMatrix @ transformMatrix

                        # 下列tmpCSIa1Raw == tmpCSIa1IndTmp
                        # tmpCSIa1Raw = extCSIa1[max_corr_index] @ publicMatrix
                        # tmpCSIb1Raw = extCSIb1[max_corr_index] @ publicMatrix
                        # np.random.seed(0)
                        # tmpCSIe1Raw = np.random.normal(np.mean(extCSIa1[max_corr_index]),
                        #                                np.std(extCSIa1[max_corr_index]),
                        #                                extCSIa1[max_corr_index].shape) @ publicMatrix

                        # 只分析低维度的转移矩阵
                        # publicMatrix = transformMatrix
                        # 分析该矩阵的随机性
                        # publicMatrix = np.random.normal(0, 1, (len(transformMatrix), len(transformMatrix)))
                        # 针对于加性对角矩阵
                        # publicMatrix = np.random.normal(0, 1, transformMatrix.shape)
                        data = publicMatrix.flatten()
                        shapiro_test = stats.shapiro(data)
                        ks_test = stats.kstest(data, 'norm', args=(np.mean(data), np.std(data)))
                        anderson_test = stats.anderson(data, dist='norm')
                        # print(publicMatrix.shape)
                        # print(f"Shapiro-Wilk Test: Statistic={round(shapiro_test.statistic, 3)}, p-value={round(shapiro_test.pvalue, 3)}")
                        # print(f"Kolmogorov-Smirnov Test: Statistic={round(ks_test.statistic, 3)}, p-value={round(ks_test.pvalue, 3)}")
                        # print(f"Anderson-Darling Test: Statistic={round(anderson_test.statistic, 3), list(anderson_test.critical_values)}")
                        sp_statistic.append(shapiro_test.statistic)
                        sp_p_value.append(shapiro_test.pvalue)
                        ks_statistic.append(ks_test.statistic)
                        ks_p_value.append(ks_test.pvalue)
                        ad_statistic.append(anderson_test.statistic)
                        ad_critical_values.append(list(anderson_test.critical_values))

                        # plt.figure()
                        # plt.imshow(randomMatrix, cmap='hot', interpolation='nearest')
                        # plt.colorbar()
                        # plt.show()
                        #
                        # plt.figure()
                        ...
                        #
                        # plt.figure()
                        # plt.imshow(publicMatrix, cmap='hot', interpolation='nearest')
                        # plt.colorbar()
                        # plt.show()
                        # exit()

                        # np.random.seed(10000)
                        # transformMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 4, transformMatrix.shape) @ transformMatrix
                        ...
                        # plt.figure()
                        # plt.plot(np.hstack(tmpCSIa1Ind))
                        # plt.plot(np.hstack(tmpCSIb1Ind))
                        # plt.plot(np.hstack(tmpCSIe1Ind))
                        # plt.show()
                        ...
                        # 分析该矩阵的随机性
                        # randomMatrix = np.random.normal(0, 1, (len(transformMatrix), len(transformMatrix)))
                        # 针对于加性对角矩阵
                        # randomMatrix = np.random.normal(0, 1, transformMatrix.shape)
                        # publicMatrix = randomMatrix + transformMatrix
                        # data = publicMatrix.flatten()
                        # shapiro_test = stats.shapiro(data)
                        # ks_test = stats.kstest(data, 'norm', args=(np.mean(data), np.std(data)))
                        ...
                        # plt.figure()
                        # plt.plot(np.hstack(tmpCSIa1IndTmp))
                        # plt.plot(np.hstack(tmpCSIb1IndTmp))
                        # plt.plot(np.hstack(tmpCSIe1IndTmp))
                        # plt.show()
                        #
                        # plt.figure()
                        # plt.imshow(transformMatrix, cmap='hot', interpolation='nearest')
                        ...
                        if withIndexValue:
                            # 将测量值和其索引结合成二维数组
                            tmpCSIa1Index = np.array(tmpCSIa1Ind).argsort().argsort()
                            tmpCSIb1Index = np.array(tmpCSIb1Ind).argsort().argsort()
                            tmpCSIe1Index = np.array(tmpCSIe1Ind).argsort().argsort()

                            index_mean_pearson_ab.append(pearsonr(tmpCSIa1Index, tmpCSIb1Index)[0])
                            value_mean_pearson_ab.append(pearsonr(tmpCSIa1Ind, tmpCSIb1Ind)[0])

                            # 将index和value放缩到同样的区间内
                            tmpCSIa1Ind = (tmpCSIa1Ind - np.min(tmpCSIa1Ind)) / (np.max(tmpCSIa1Ind) - np.min(tmpCSIa1Ind))
                            tmpCSIb1Ind = (tmpCSIb1Ind - np.min(tmpCSIb1Ind)) / (np.max(tmpCSIb1Ind) - np.min(tmpCSIb1Ind))
                            tmpCSIe1Ind = (tmpCSIe1Ind - np.min(tmpCSIe1Ind)) / (np.max(tmpCSIe1Ind) - np.min(tmpCSIe1Ind))
                            ...
                            tmpCSIa1Ind = np.array(list(zip(tmpCSIa1Ind, tmpCSIa1Index)))
                            tmpCSIb1Ind = np.array(list(zip(tmpCSIb1Ind, tmpCSIb1Index)))
                            tmpCSIe1Ind = np.array(list(zip(tmpCSIe1Ind, tmpCSIe1Index)))

                            both_mean_pearson_ab.append(
                                pearsonr(np.array(tmpCSIa1Ind).flatten(), np.array(tmpCSIb1Ind).flatten())[0])
                            both_mean_pearson_ae.append(
                                pearsonr(np.array(tmpCSIa1Ind).flatten(), np.array(tmpCSIe1Ind).flatten())[0])
                            ...
                            tmpCSIa1Ind = tmpCSIa1Ind * tmpCSIa1Index
                            tmpCSIb1Ind = tmpCSIb1Ind * tmpCSIb1Index
                            tmpCSIe1Ind = tmpCSIe1Ind * tmpCSIe1Index
                        else:
                            if withoutSort:
                                tmpCSIa1Ind = np.array(tmpCSIa1Ind)
                                tmpCSIb1Ind = np.array(tmpCSIb1Ind)
                                tmpCSIe1Ind = np.array(tmpCSIe1Ind)
                            else:
                ...
                print("both_mean_pearson_ae", np.mean(both_mean_pearson_ae))
                ...
                print("sp_statistic", round(np.mean(sp_statistic), 3))
                print("sp_p_value", round(np.mean(sp_p_value), 3))
                print("ks_statistic", round(np.mean(ks_statistic), 3))
                print("ks_p_value", round(np.mean(ks_p_value), 3))
                print("ad_statistic", round(np.mean(ad_statistic), 3))
                print("ad_critical_values", np.round(np.mean(ad_critical_values, axis=0), 3))
                ...
                print("\033[0;34;40m", round(np.mean(entropy_a), 3), round(np.mean(corr_ab), 5),
                      round(np.mean(both_mean_pearson_ab), 5),  round(np.mean(max_min_dtw_ab), 3), "\033[0m")
                ...
                if isPartPerm:
                    print("with part permutation")
                else:
                    print("without part permutation")
