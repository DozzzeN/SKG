            ...
            for withoutSort in withoutSorts:
                ...
                #     staInd += keyLenLoop
                    ...
                    if segment_option == "find_search":
                        ...
                        # tmpCSIa1Before = extCSIa1[max_corr_index] @ randomMatrix
                        # tmpCSIb1Before = extCSIb1[max_corr_index] @ randomMatrix
                        # tmpCSIe1Before = extCSIe1[max_corr_index] @ randomMatrix
                        # tmpCSIa1Before = tmpCSIa1Before - np.mean(tmpCSIa1Before)
                        # tmpCSIb1Before = tmpCSIb1Before - np.mean(tmpCSIb1Before)
                        # tmpCSIe1Before = tmpCSIe1Before - np.mean(tmpCSIe1Before)
                        ...
                        if withIndexValue:
                            ...
                            transformMatrix = np.linalg.lstsq(tmpCSIa1Ind.T.reshape(1, -1), tmpCSIa1IndTmp.T.reshape(1, -1), rcond=None)[0]
                            tmpCSIa1IndTmp = np.matmul(tmpCSIa1Ind.T, transformMatrix).T
                            tmpCSIb1IndTmp = np.matmul(tmpCSIb1Ind.T, transformMatrix).T
                            tmpCSIe1IndTmp = np.matmul(tmpCSIe1Ind.T, transformMatrix).T
                        ...
                        # 不同矩阵维度的转换
                        # 4 * 4
                        # transformMatrix = np.linalg.lstsq(np.array(np.hstack(tmpCSIa1Ind)).reshape(segNum * 2, segLen),
                        #                                   np.array(np.hstack(tmpCSIa1IndTmp)).reshape(segNum * 2, segLen), rcond=None)[0]
                        # if isExtraPrint:
                        #     print(transformMatrix.shape)
                        # tmpCSIa1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIa1Ind).reshape(segNum * 2, segLen).T).reshape(keyLen, 2)
                        # tmpCSIb1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIb1Ind).reshape(segNum * 2, segLen).T).reshape(keyLen, 2)
                        # tmpCSIe1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIe1Ind).reshape(segNum * 2, segLen).T).reshape(keyLen, 2)

                        # 16 * 16
                        # transformMatrix = np.linalg.lstsq(np.array(np.hstack(tmpCSIa1Ind)).reshape(int(segNum / 2), segLen * 4),
                        #                                   np.array(np.hstack(tmpCSIa1IndTmp)).reshape(int(segNum / 2), segLen * 4), rcond=None)[0]
                        # if isExtraPrint:
                        #     print(transformMatrix.shape)
                        # tmpCSIa1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIa1Ind).reshape(int(segNum / 2), segLen * 4).T).reshape(keyLen, 2)
                        # tmpCSIb1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIb1Ind).reshape(int(segNum / 2), segLen * 4).T).reshape(keyLen, 2)
                        # tmpCSIe1IndTmp = np.array(transformMatrix @ np.hstack(tmpCSIe1Ind).reshape(int(segNum / 2), segLen * 4).T).reshape(keyLen, 2)
