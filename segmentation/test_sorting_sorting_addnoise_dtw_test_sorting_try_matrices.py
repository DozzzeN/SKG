        ...
        for withoutSort in withoutSorts:
            ...
            while staInd < dataLenLoop:
                ...
                if withNewMatrix:
                    ...
                    np.random.seed(staInd)
                    # randomMatrix += np.random.normal(np.mean(randomMatrix), np.std(randomMatrix), (keyLen, keyLen))
                    ...
                    both_mean_with_matrix_pearson_ae.append(pearsonr(tmpCSIa1, tmpCSIe1)[0])

                    # print("corr", pearsonr(tmpCSIa1, tmpCSIb1)[0])
                    # print("e corr", pearsonr(tmpCSIa1, tmpCSIe1)[0])

                    # rawCSIa1 = CSIa1Orig[range(staInd, endInd, 1)]
                    # rawCSIa1 = rawCSIa1 - np.mean(rawCSIa1)
                    # randomMatrix = np.matmul(np.linalg.inv(rawCSIa1), tmpCSIa1)
                    # print("乘性矩阵方差", np.var(randomMatrix))
                # 寻找最优噪声矩阵
                if withSortMatrix:
                    target = list(range(keyLen))
                    ...
                    for i in range(keyLen):
                        ...
                        for j in range(len(target)):
                            dist = min(dist, abs(tmpCSIa1[i] - target[j]))
                        for j in range(len(target)):
                            if abs(tmpCSIa1[i] - target[j]) == dist:
                                resCSIa1.append(target[j])
                                target[j] = np.inf
                ...
                if withOptSortMatrix:
                    ...
                    Ka = search_random_matrix_uniform(tmpCSIa1, "l-bfgs-b", 1)

                    Ka = Ka - np.mean(Ka)
                    Ka = (Ka - np.min(Ka)) / (np.max(Ka) - np.min(Ka))

                    # plt.figure()
                    # plt.plot(np.sort(tmpCSIa1))
                    # plt.plot(np.sort(tmpCSIb1))
                    # plt.plot(np.sort(tmpCSIe1))
                    # plt.show()

                    tmpCSIa1 = np.matmul(tmpCSIa1, Ka)
                    tmpCSIb1 = np.matmul(tmpCSIb1, Ka)
                    tmpCSIe1 = np.matmul(tmpCSIe1, Ka)
                ...
                if withOptMatrix:
                    tau = 10
                    # Ka = optimize_random_matrix(tmpCSIa1, tau)
                    # Kb = optimize_random_matrix(tmpCSIb1, tau)
                    # Ke = optimize_random_matrix(tmpCSIe1, tau)
                    ...
                    # Ka = optimize_random_matrix_max_min(tmpCSIa1, tau)
                    # Kb = optimize_random_matrix_max_min(tmpCSIb1, tau)
                    # Ke = optimize_random_matrix_max_min(tmpCSIe1, tau)

                    # 各自搜索随机矩阵效果不好，准确率只有65.8%
                    # 用相同搜索随机矩阵准确率可达99.42%，但攻击者猜测率有61%
                    Ka = search_random_matrix_max_min(tmpCSIa1, tau)
                    Kb = search_random_matrix_max_min(tmpCSIb1, tau)
                    Ke = search_random_matrix_max_min(tmpCSIe1, tau)

                    tmpCSIa1 = np.matmul(tmpCSIa1, Ka)
                    tmpCSIb1 = np.matmul(tmpCSIb1, Kb)
                    tmpCSIe1 = np.matmul(tmpCSIe1, Ke)
                ...
                if segment_option == "opt_pair":
                    ...
                    for i in range(trials):
                        ...
                        tmpCSIa1IndReshape_tmp = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                        ...
                        #                                segment_sequence(tmpCSIb1Inds[i], segment_method_ori_tmp))
