...
def generate_condition_segment_lengths(segments, min_length, max_length, total_length, seed):
    segment_lengths = []
    remaining_length = total_length
    for _ in range(segments - 1):
        if remaining_length <= 1:
...
def find_min_threshold(dataA, dataB, segment_method):
    ...
    for i in range(len(segment_method) - 1):
        ...
        min_threshold = min(min_threshold, abs(offsets[next_segment_pos] - offsets[next_segment_pos - 1]))
...
for segment in range(4):
    ...
    # segment_length = generate_segment_lengths(M, M * N, segment * 100)
    # segment_length = generate_condition_segment_lengths(segment_len, M - 2, M + 2, M * N, segment * 100)
    segment_length = find_sub_opt_segment_method(data1, M - 2, segment_len)
    print("segment_length A:", segment_length)
    print("segment_length B:", find_sub_opt_segment_method(data2, M - 2, segment_len))
    # 对分段进行随机置换
    if fixSeed:
        np.random.seed(segment * 1000)
    segment_length = np.random.permutation(segment_length)
    print("segment_length: ", segment_length)
    data1 = segment_data(data1, segment_len, segment_length)
    data2 = segment_data(data2, segment_len, segment_length)

    print("data1: ", [list(data) for data in data1])
    print("data2: ", [list(data) for data in data2])

    permutation = list(range(segment_len))
    ...
    # 测自相关来避免连续密钥
    # cross_correlation_of_B = discrete_corr(data1_flat, data2_flat, threshold)
    # cross_correlation_of_A = discrete_corr(data2_back, data2_flat, threshold)
    # 固定step的窗求解互相关
    ...
    height = 2
    segment_peaks_A = find_peaks_in_segments(cross_correlation_of_A, height, 2 * step + 1)[1]
    segment_peaks_B = find_peaks_in_segments(cross_correlation_of_B, height - 1, 2 * step + 1)[1]
    ...
    print("search_segment_method_with_offset")
    segment_method_A = search_segment_method_with_offset(data2_back, data2_flat, 2, 2)
    min_threshold = find_min_threshold(data2_back, data2_flat, segment_method_A)
    # segment_method_B = search_segment_method_with_offset(data1_flat, data2_flat, 2, 2)
    segment_method_B = search_segment_method_with_offset(data1_flat, data2_flat, 2, min_threshold)
    ...
    print(segment_method_A)
    print(segment_method_B)
    print("matching")
    print(search_segment_method(data2_back, data2_flat, np.array(segment_method_A).astype(int), step, threshold))
    print(search_segment_method(data1_flat, data2_flat, np.array(segment_method_B).astype(int), step, threshold))
...
#     local_maxima = []
#     step = N
#     for j in range(step, len(cross_correlations[i]) - step):
#         if cross_correlations[i][j] >= cross_correlations[i][j - step:j + step + 1].max():
#             local_maxima.append(j)
#     print("i", i, "local_maxima: ", local_maxima, cross_correlations[i][local_maxima].tolist())
#     plt.tight_layout()
