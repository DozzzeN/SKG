        ...
        for withoutSort in withoutSorts:
            ...
            while staInd < dataLenLoop:
                ...
                if segment_option == "opt_pair":
                    ...
                    if addNoise == "" and isRunBack is False:
                        np.random.seed(10000)
                    ...
                    if isRunBack:
                        np.random.seed(10000 + staInd + runBackCounts)
                        transformMatrix += np.random.uniform(0, np.std(CSIa1Orig) * 4, transformMatrix.shape)
                    ...
                    tmpCSIb1EvenSegment = np.array(tmpCSIb1Ind).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIb1Ind))
                    tmpCSIe1EvenSegment = np.array(tmpCSIe1Ind).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIe1Ind))
                    dist_opt_seg = compute_min_dtw(tmpCSIa1IndReshape, tmpCSIb1IndReshape)
                    dist_even_seg = compute_min_dtw(tmpCSIa1EvenSegment, tmpCSIb1EvenSegment)
                    if (dist_opt_seg < dist_even_seg):
                        print("staInd", staInd, "最优分段最大差距", dist_opt_seg, "平均分段最大差距", dist_even_seg)
