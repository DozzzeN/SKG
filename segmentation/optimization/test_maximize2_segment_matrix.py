...
# 定义目标函数
def objective(K, A, n, segment_method):
...
# 随机生成向量 A
np.random.seed(0)
A_bck = np.random.normal(0, 1, n)  # 生成一个 1 x n 的向量
...
for optimizer in optimizers:
    ...
    result = minimize(objective, initial_K, args=(A, n, segment_method), method=optimizer)
    ...
    # 可视化 K2 + 噪音
    optimal_K2n = optimal_K + np.random.normal(np.mean(optimal_K), np.std(optimal_K), (n, n))
