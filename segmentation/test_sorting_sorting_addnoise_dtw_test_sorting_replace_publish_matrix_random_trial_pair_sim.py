...
from segmentation.methods import find_opt_segment_method, segment_sequence, compute_min_dtw, \
    ...
    replace_segments_position_compared_with_all_segments_no_limit, entropyPerm, \
    replace_segments_position_compared_with_all_segments_no_limit_plus
...
for f in fileName:
    for addNoise in addNoises:
        for withoutSort in withoutSorts:
            ...
            # 首次置换
            if segNum == 4 or segNum == 8 or segNum == 16:
            ...
            staInd = -keyLenLoop
            # 以下索引位置回退一次后正确：1680 2800 5632 6448
            ...
            while staInd < dataLenLoop:
                ...
                if isRunBack:
                    ...
                    for i in range(trials):
                        # np.random.seed((i * staInd * runBackCounts * 100000) % (2**32 - 1))
                        np.random.seed((i + staInd + runBackCounts))
                        randomMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        tmpCSIa1s.append(np.matmul(tmpCSIa1, randomMatrix))
                        tmpCSIb1s.append(np.matmul(tmpCSIb1, randomMatrix))
                        tmpCSIe1s.append(np.matmul(tmpCSIe1, randomMatrix))
                ...
                if addNoise == "add" and isRunBack is False:
                    ...
                    randomMatrices = []
                    for i in range(trials):
                        # np.random.seed((i * 10000 + 100000) % (2**32 - 1))
                        np.random.seed(i + 10000)
                        randomMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        # randomMatrix = np.random.exponential(np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        # randomMatrix = np.random.normal(0, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        # randomMatrix = np.random.laplace(0, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        # randomMatrix = np.random.poisson(np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        # randomMatrix = np.random.rayleigh(np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        # randomMatrix = np.random.gamma(1, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                        randomMatrices.append(randomMatrix)
                    # randomMatrix = np.random.uniform(0, 1, size=(keyLen, keyLen))
                    ...
                    for i in range(trials):
                        ...
                        # tmpCSIa1P = np.matmul(tmpCSIa1, randomMatrices[i])[shuffleInd]
                        # tmpCSIb1P = np.matmul(tmpCSIb1, randomMatrices[i])[shuffleInd]
                        # tmpCSIe1P = np.matmul(tmpCSIe1, randomMatrices[i])[shuffleInd]

                        tmpCSIa1P = np.matmul(tmpCSIa1, randomMatrices[i])
                        tmpCSIb1P = np.matmul(tmpCSIb1, randomMatrices[i])
                        tmpCSIe1P = np.matmul(tmpCSIe1, randomMatrices[i])

                        # 交织
                        # tmpCSIa1P, tmpCSIb1P, tmpCSIe1P = entropyPerm(tmpCSIa1P, tmpCSIb1P, tmpCSIe1P, keyLen, i)

                        tmpCSIa1s.append(tmpCSIa1P)
                        tmpCSIb1s.append(tmpCSIb1P)
                        tmpCSIe1s.append(tmpCSIe1P)
                ...
                simCSIa1s = []
                for i in range(trials):
                    np.random.seed(staInd + trials)
                    addMatrix = np.random.normal(0, 0.1, keyLen)
                    simCSIa1s.append(tmpCSIa1 + addMatrix)
                ...
                if withIndexValue:
                    ...
                    for i in range(trials):
                        # 置换
                        tmpCSIa1Index = np.array(tmpCSIa1s[i]).argsort().argsort()
                        tmpCSIb1Index = np.array(tmpCSIb1s[i]).argsort().argsort()
                        tmpCSIe1Index = np.array(tmpCSIe1s[i]).argsort().argsort()

                        simCSIa1Index = np.array(simCSIa1s[i]).argsort().argsort()
                        ...
                        tmpCSIa1s[i] = (tmpCSIa1s[i] - np.min(tmpCSIa1s[i])) / (np.max(tmpCSIa1s[i]) - np.min(tmpCSIa1s[i]))
                        tmpCSIb1s[i] = (tmpCSIb1s[i] - np.min(tmpCSIb1s[i])) / (np.max(tmpCSIb1s[i]) - np.min(tmpCSIb1s[i]))
                        tmpCSIe1s[i] = (tmpCSIe1s[i] - np.min(tmpCSIe1s[i])) / (np.max(tmpCSIe1s[i]) - np.min(tmpCSIe1s[i]))
                        ...
                        simCSIa1s[i] = (simCSIa1s[i] - np.min(simCSIa1s[i])) / (np.max(simCSIa1s[i]) - np.min(simCSIa1s[i]))
                        simCSIa1Index = (simCSIa1Index - np.min(simCSIa1Index)) / (np.max(simCSIa1Index) - np.min(simCSIa1Index))

                        if isMean:
                            tmpCSIa1s[i] = tmpCSIa1s[i] - np.mean(tmpCSIa1s[i])
                            tmpCSIb1s[i] = tmpCSIb1s[i] - np.mean(tmpCSIb1s[i])
                            tmpCSIe1s[i] = tmpCSIe1s[i] - np.mean(tmpCSIe1s[i])
                            ...
                            simCSIa1s[i] = simCSIa1s[i] - np.mean(simCSIa1s[i])
                            simCSIa1Index = simCSIa1Index - np.mean(simCSIa1Index)

                        tmpCSIa1Inds.append(np.array(list(zip(tmpCSIa1s[i], tmpCSIa1Index))))
                        tmpCSIb1Inds.append(np.array(list(zip(tmpCSIb1s[i], tmpCSIb1Index))))
                        tmpCSIe1Inds.append(np.array(list(zip(tmpCSIe1s[i], tmpCSIe1Index))))

                        simCSIa1Inds.append(np.array(list(zip(simCSIa1s[i], simCSIa1Index))))
                ...
                if segment_option == "opt_pair":
                    ...
                    min_index_dist_opt_seg = np.inf

                    max_dist_opt_seg = -np.inf
                    max_corr_flatten = -np.inf
                    max_all_dist_opt_seg = -np.inf
                    min_kuro_flatten = np.inf
                    min_skew_flatten = np.inf
                    tmpCSIa1Ind = []
                    tmpCSIb1Ind = []
                    tmpCSIe1Ind = []
                    simCSIa1Ind = []
                    ...
                    for i in range(trials):
                        # 滑动窗口分段
                        if withIndexValue:
                            segment_method_ori_tmp = find_sub_opt_segment_method_sliding(
                                tmpCSIa1Inds[i], tmpCSIa1Inds[i], 3, 5)
                        else:
                            if withoutSort is True:
                                segment_method_ori_tmp = find_sub_opt_segment_method_sliding(
                                    tmpCSIa1Inds[i], tmpCSIa1Inds[i], 3, 5)
                                # segment_method_ori_tmp = find_sub_opt_segment_method_sliding_threshold(
                                #     tmpCSIa1Ind, tmpCSIa1Ind, 3, 5, 0.005)
                            else:
                                segment_method_ori_tmp = find_sub_opt_segment_method_sliding_threshold(
                                    tmpCSIa1Inds[i], tmpCSIa1Inds[i], 3, 5, 60)

                        # 是否过滤
                        if isFilter:
                            if sum(segment_method_ori_tmp) != keyLen:
                                continue

                        tmpCSIa1IndReshapeTmp = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                        tmpCSIb1IndReshapeTmp = segment_sequence(tmpCSIb1Inds[i], segment_method_ori_tmp)
                        tmpCSIe1IndReshapeTmp = segment_sequence(tmpCSIe1Inds[i], segment_method_ori_tmp)

                        # print("max dtw of ab", compute_max_index_dtw(tmpCSIa1IndReshapeTmp, tmpCSIb1IndReshapeTmp))
                        #
                        # print("max min dtw of ab / aa / bb",
                        #       compute_min_dtw(tmpCSIa1IndReshapeTmp, tmpCSIb1IndReshapeTmp),
                        #       compute_min_dtw(tmpCSIa1IndReshapeTmp, tmpCSIa1IndReshapeTmp),
                        #       compute_min_dtw(tmpCSIb1IndReshapeTmp, tmpCSIb1IndReshapeTmp))

                        # 替换原始数据
                        max_same_position_dist = compute_min_dtw(tmpCSIa1IndReshapeTmp, tmpCSIb1IndReshapeTmp)
                        ...
                        tmpCSIa1IndReshapeTmp, modifications = replace_segments_position_compared_with_all_segments_no_limit(
                            tmpCSIa1IndReshapeTmp, segLen * 2, staInd + 100000)
                        # tmpCSIa1IndReshapeTmp, modifications = replace_segments_position_compared_with_all_segments_no_limit_plus(
                        #     tmpCSIa1IndReshapeTmp, segLen * 2)

                        before_add_mean_pearson_ab.append(abs(pearsonr(np.hstack(tmpCSIa1Inds[i]), np.hstack(tmpCSIb1Inds[i]))[0]))
                        before_add_mean_pearson_ae.append(abs(pearsonr(np.hstack(tmpCSIa1Inds[i]), np.hstack(tmpCSIe1Inds[i]))[0]))

                        if withIndexValue:
                            tmpCSIa1IndTmp = np.vstack(tmpCSIa1IndReshapeTmp)
                        else:
                            tmpCSIa1IndTmp = np.hstack((tmpCSIa1IndReshapeTmp))

                        transformMatrix = np.linalg.lstsq(tmpCSIa1Inds[i].T, tmpCSIa1IndTmp.T, rcond=None)[0]

                        # 似乎更好一些 (4 * 8 0.999203912 0.997542998 0.675138206 0.674600737 / 0.505856932 0.004914005 26/407=0.063882)
                        # np.random.seed(10000)
                        # randomMatrix = np.random.uniform(0, np.std(CSIa1Orig) * 4, transformMatrix.shape)
                        ...
                        tmpCSIa1IndTmp = np.matmul(tmpCSIa1Inds[i].T, transformMatrix).T
                        tmpCSIb1IndTmp = np.matmul(tmpCSIb1Inds[i].T, transformMatrix).T
                        tmpCSIe1IndTmp = np.matmul(tmpCSIe1Inds[i].T, transformMatrix).T

                        simCSIa1IndTmp = np.matmul(simCSIa1Inds[i].T, transformMatrix).T
                        ...
                        tmpCSIa1IndBck = tmpCSIa1Inds[i]
                        tmpCSIb1IndBck = tmpCSIb1Inds[i]
                        tmpCSIe1IndBck = tmpCSIe1Inds[i]
                        tmpCSIa1Inds[i] = tmpCSIa1IndTmp
                        tmpCSIb1Inds[i] = tmpCSIb1IndTmp
                        tmpCSIe1Inds[i] = tmpCSIe1IndTmp

                        simCSIa1IndBck = simCSIa1Inds[i]
                        simCSIa1Inds[i] = simCSIa1IndTmp
                        ...
                        if min(segment_method_ori_tmp) < 3:
                            print("小于最小长度")
                            continue
                        ...
                        simCSIa1IndReshapeTmp = segment_sequence(simCSIa1Inds[i], segment_method_ori_tmp)

                        # 补全至keyLen长度用于等长分段
                        if len(tmpCSIa1Inds[i]) < keyLen:
                            if withIndexValue:
                                tmpCSIa1Inds[i] = np.vstack((tmpCSIa1Inds[i], tmpCSIa1IndBck[-(keyLen - len(tmpCSIa1Inds[i])):]))
                                tmpCSIb1Inds[i] = np.vstack((tmpCSIb1Inds[i], tmpCSIb1IndBck[-(keyLen - len(tmpCSIb1Inds[i])):]))
                                tmpCSIe1Inds[i] = np.vstack((tmpCSIe1Inds[i], tmpCSIe1IndBck[-(keyLen - len(tmpCSIe1Inds[i])):]))

                                simCSIa1Inds[i] = np.vstack((simCSIa1Inds[i], simCSIa1IndBck[-(keyLen - len(simCSIa1Inds[i])):]))
                            else:
                                tmpCSIa1Inds[i] = np.hstack((tmpCSIa1Inds[i], tmpCSIa1IndBck[-(keyLen - len(tmpCSIa1Inds[i])):]))
                                tmpCSIb1Inds[i] = np.hstack((tmpCSIb1Inds[i], tmpCSIb1IndBck[-(keyLen - len(tmpCSIb1Inds[i])):]))
                                tmpCSIe1Inds[i] = np.hstack((tmpCSIe1Inds[i], tmpCSIe1IndBck[-(keyLen - len(tmpCSIe1Inds[i])):]))

                                simCSIa1Inds[i] = np.hstack((simCSIa1Inds[i], simCSIa1IndBck[-(keyLen - len(simCSIa1Inds[i])):]))

                        tmpCSIa1EvenSegment = np.array(tmpCSIa1Inds[i]).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIa1Inds[i]))
                        tmpCSIb1EvenSegment = np.array(tmpCSIb1Inds[i]).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIb1Inds[i]))
                        tmpCSIe1EvenSegment = np.array(tmpCSIe1Inds[i]).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIe1Inds[i]))

                        simCSIa1EvenSegment = np.array(simCSIa1Inds[i]).reshape(int(keyLen / segLen), segLen, np.ndim(simCSIa1Inds[i]))

                        dist_opt_seg = compute_min_dtw(tmpCSIa1IndReshapeTmp, tmpCSIb1IndReshapeTmp)
                        dist_even_seg = compute_min_dtw(tmpCSIa1EvenSegment, tmpCSIb1EvenSegment)
                        index_dist_opt_seg = compute_max_index_dtw(tmpCSIa1IndReshapeTmp, tmpCSIb1IndReshapeTmp)
                        index_dist_even_seg = compute_max_index_dtw(tmpCSIa1EvenSegment, tmpCSIb1EvenSegment)

                        sim_dist_opt_seg = compute_min_dtw(tmpCSIa1IndReshapeTmp, simCSIa1IndReshapeTmp)
                        sim_dist_even_seg = compute_min_dtw(tmpCSIa1EvenSegment, simCSIa1EvenSegment)
                        sim_index_dist_opt_seg = compute_max_index_dtw(tmpCSIa1IndReshapeTmp, simCSIa1IndReshapeTmp)
                        sim_index_dist_even_seg = compute_max_index_dtw(tmpCSIa1EvenSegment, simCSIa1EvenSegment)
                        # dist_opt_seg = compute_min_dtw(tmpCSIa1IndReshape_tmp,
                        #                                segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp))
                        # dist_even_seg = compute_min_dtw(tmpCSIa1EvenSegment,
                        #                                 np.array(tmpCSIa1Inds[i]).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIa1Inds[i])))

                        # all_dist_opt_seg = np.sum(compute_all_dtw(segment_sequence(tmpCSIa1Ind, segment_method_ori),
                        #                           segment_sequence(tmpCSIb1Inds[i], segment_method_ori)))
                        # all_dist_even_seg = np.sum(compute_all_dtw(tmpCSIa1EvenSegment,
                        #     np.array(tmpCSIb1Inds[i]).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIb1Inds[i]))))

                        # all_dist_opt_seg = np.sum(compute_all_dtw(segment_sequence(tmpCSIa1Ind, segment_method_ori),
                        #                           segment_sequence(tmpCSIa1Inds[i], segment_method_ori)))
                        # all_dist_even_seg = np.sum(compute_all_dtw(tmpCSIa1EvenSegment,
                        #     np.array(tmpCSIa1Inds[i]).reshape(int(keyLen / segLen), segLen, np.ndim(tmpCSIa1Inds[i]))))

                        # corr_flatten = pearsonr(tmpCSIa1Inds[i].reshape(1, -1)[0], tmpCSIb1Inds[i].reshape(1, -1)[0])[0]
                        # print("dist_opt_seg", dist_opt_seg, "dist_even_seg", dist_even_seg)

                        # kuro_flatten = kurtosis(tmpCSIa1Inds[i].reshape(-1, 1))

                        # skew_flatten = abs(skew(tmpCSIa1Inds[i].reshape(-1, 1)))

                        # if skew_flatten < min_skew_flatten:
                        #     tmpCSIa1Ind = tmpCSIa1Inds[i]
                        #     tmpCSIb1Ind = tmpCSIb1Inds[i]
                        #     tmpCSIe1Ind = tmpCSIe1Inds[i]
                        #     min_skew_flatten = skew_flatten
                        #     tmpCSIa1IndReshape = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                        #     segment_method_ori = segment_method_ori_tmp
                        #
                        #     min_length = min(segment_method_ori)
                        #     max_length = max(segment_method_ori)
                        #     num_segments = len(segment_method_ori)
                        #     measurements_len = sum(segment_method_ori)
                        # if kuro_flatten < min_kuro_flatten:
                        #     tmpCSIa1Ind = tmpCSIa1Inds[i]
                        #     tmpCSIb1Ind = tmpCSIb1Inds[i]
                        #     tmpCSIe1Ind = tmpCSIe1Inds[i]
                        #     min_kuro_flatten = kuro_flatten
                        #     tmpCSIa1IndReshape = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                        #     segment_method_ori = segment_method_ori_tmp
                        #
                        #     min_length = min(segment_method_ori)
                        ...
                        #     num_segments = len(segment_method_ori)
                        #     measurements_len = sum(segment_method_ori)
                        # if corr_flatten > max_corr_flatten:
                        #     tmpCSIa1Ind = tmpCSIa1Inds[i]
                        #     tmpCSIb1Ind = tmpCSIb1Inds[i]
                        #     tmpCSIe1Ind = tmpCSIe1Inds[i]
                        #     max_corr_flatten = corr_flatten
                        #     tmpCSIa1IndReshape = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                        #     segment_method_ori = segment_method_ori_tmp
                        #
                        #     min_length = min(segment_method_ori)
                        ...
                        #
                        # 真实数据
                        # if dist_opt_seg > max_dist_opt_seg and dist_opt_seg > index_dist_opt_seg:
                        #     isModify = True
                        #     tmpCSIa1Ind = tmpCSIa1Inds[i]
                        #     tmpCSIb1Ind = tmpCSIb1Inds[i]
                        #     tmpCSIe1Ind = tmpCSIe1Inds[i]
                        #     max_dist_opt_seg = dist_opt_seg
                        #     tmpCSIa1IndReshape = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                        #     segment_method_ori = segment_method_ori_tmp
                        #
                        #     min_length = min(segment_method_ori)
                        ...
                        #     num_segments = len(segment_method_ori)
                        #     measurements_len = sum(segment_method_ori)
                        # if dist_even_seg > max_dist_opt_seg and dist_even_seg > index_dist_even_seg:
                        #     isModify = True
                        #     tmpCSIa1Ind = tmpCSIa1Inds[i]
                        #     tmpCSIb1Ind = tmpCSIb1Inds[i]
                        #     tmpCSIe1Ind = tmpCSIe1Inds[i]
                        #     max_dist_opt_seg = dist_even_seg
                        ...
                        if dist_opt_seg > max_dist_opt_seg and dist_opt_seg > sim_index_dist_opt_seg:
                            isModify = True
                            tmpCSIa1Ind = tmpCSIa1Inds[i]
                            tmpCSIb1Ind = tmpCSIb1Inds[i]
                            tmpCSIe1Ind = tmpCSIe1Inds[i]
                            ...
                            num_segments = len(segment_method_ori)
                            measurements_len = sum(segment_method_ori)
                        if dist_even_seg > max_dist_opt_seg and dist_even_seg > sim_index_dist_even_seg:
                            isModify = True
                            tmpCSIa1Ind = tmpCSIa1Inds[i]
                            tmpCSIb1Ind = tmpCSIb1Inds[i]
                            tmpCSIe1Ind = tmpCSIe1Inds[i]
                            ...
                            num_segments = len(segment_method_ori)
                            measurements_len = sum(segment_method_ori)
                        if sim_dist_opt_seg > max_dist_opt_seg and sim_dist_opt_seg > sim_index_dist_opt_seg:
                            isModify = True
                            tmpCSIa1Ind = tmpCSIa1Inds[i]
                            tmpCSIb1Ind = tmpCSIb1Inds[i]
                            tmpCSIe1Ind = tmpCSIe1Inds[i]
                            max_dist_opt_seg = sim_dist_opt_seg
                            tmpCSIa1IndReshape = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                            segment_method_ori = segment_method_ori_tmp
                            ...
                            num_segments = len(segment_method_ori)
                            measurements_len = sum(segment_method_ori)
                        if sim_dist_even_seg > max_dist_opt_seg and sim_dist_even_seg > sim_index_dist_even_seg:
                            isModify = True
                            tmpCSIa1Ind = tmpCSIa1Inds[i]
                            tmpCSIb1Ind = tmpCSIb1Inds[i]
                            tmpCSIe1Ind = tmpCSIe1Inds[i]
                            max_dist_opt_seg = sim_dist_even_seg
                            tmpCSIa1IndReshape = tmpCSIa1EvenSegment
                            segment_method_ori = [segLen] * int(keyLen / segLen)
                        ...
                        # if all_dist_opt_seg > max_all_dist_opt_seg:
                        #     tmpCSIa1Ind = tmpCSIa1Inds[i]
                        #     tmpCSIb1Ind = tmpCSIb1Inds[i]
                        #     tmpCSIe1Ind = tmpCSIe1Inds[i]
                        #     max_all_dist_opt_seg = all_dist_opt_seg
                        #     tmpCSIa1IndReshape = segment_sequence(tmpCSIa1Inds[i], segment_method_ori_tmp)
                        #     segment_method_ori = segment_method_ori_tmp
                        #
                        #     min_length = min(segment_method_ori)
                        ...
                        #     num_segments = len(segment_method_ori)
                        #     measurements_len = sum(segment_method_ori)
                        # if all_dist_even_seg > max_all_dist_opt_seg:
                        #     tmpCSIa1Ind = tmpCSIa1Inds[i]
                        #     tmpCSIb1Ind = tmpCSIb1Inds[i]
                        #     tmpCSIe1Ind = tmpCSIe1Inds[i]
                        #     max_all_dist_opt_seg = all_dist_even_seg
                        #     tmpCSIa1IndReshape = tmpCSIa1EvenSegment
                        #     segment_method_ori = [segLen] * int(keyLen / segLen)
                        #
                        #     min_length = min(segment_method_ori)
                    ...
                    if isModify is False:
                        ...
                        measurements_len = sum(segment_method_ori)
                    print("staInd", staInd, "segment_method", len(segment_method_ori), segment_method_ori)
                    tmpCSIa1IndReshapeTmp = segment_sequence(tmpCSIa1Ind, segment_method_ori)
                    tmpCSIb1IndReshapeTmp = segment_sequence(tmpCSIb1Ind, segment_method_ori)
                    print("max dtw of ab", compute_max_index_dtw(tmpCSIa1IndReshapeTmp, tmpCSIb1IndReshapeTmp))
