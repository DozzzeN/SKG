        ...
        for withoutSort in withoutSorts:
            ...
            while staInd < dataLenLoop:
                ...
                if isRunBack:
                    ...
                    # 乘随机矩阵
                    if runBackCounts2 != -1:
                        np.random.seed(staInd + runBackCounts2)
                    else:
                        np.random.seed(staInd + runBackCounts)
                        runBackCounts += 1
                ...
                if withNewMatrix:
                    ...
                    # randomMatrix = np.array(resCSIa1) / np.array(tmpCSIa1)
                    # randomMatrix = np.reshape(randomMatrix, (1, -1))[0]
                    ...
                    # a与e相关性 0.5537608855138391 攻击者比特率 0.524707657
                    # randomMatrix *= np.random.normal(np.mean(randomMatrix), np.std(randomMatrix), (keyLen, keyLen))
                    # a与e相关性 0.18851897859928882 攻击者比特率 0.496837358
                    # randomMatrix *= np.random.normal(0, 1, (keyLen, keyLen))
                    # randomMatrix = randomMatrix @ np.random.normal(0, 1, (keyLen, keyLen))
                    # a与e相关性 0.40883963967679104 攻击者比特率 0.51671227
                    # randomMatrix *= np.random.uniform(0, 1, (keyLen, keyLen))
                    # a与e相关性 0.19022176166394353 攻击者比特率 0.500191477
                    # randomMatrix *= np.random.uniform(-1, 1, (keyLen, keyLen))

                    # a与e相关性 0.19507817223565913 攻击者比特率 0.497217676
                    randomMatrix += np.random.normal(0, 1, (keyLen, keyLen))
                    # a与e相关性 0.2671304600877478 攻击者比特率 0.500721809
                    # randomMatrix += np.random.uniform(-1, 1, (keyLen, keyLen))
                    # randomMatrix += np.random.uniform(0, 1, (keyLen, keyLen))
                    # randomMatrix += np.random.uniform(0, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                    # randomMatrix = randomMatrix - np.mean(randomMatrix)
                    ...
                    both_mean_with_matrix_pearson_ae.append(abs(pearsonr(tmpCSIa1, tmpCSIe1)[0]))
                ...
                if withNewMatrix2:
                    ...
                    randomMatrix = generate_matrix_and_solve_diag(tmpCSIa1, resCSIa1)
                    # randomMatrix = randomMatrix - np.mean(randomMatrix)
                    # randomMatrix = (randomMatrix - np.min(randomMatrix)) / (np.max(randomMatrix) - np.min(randomMatrix))
                    np.random.seed(staInd)
                ...
                if withSortMatrix:
                    ...
                    for i in range(keyLen):
                        ...
                        dist = np.inf
                        for j in range(len(target)):
                            dist = min(dist, abs(tmpCSIb1[i] - target[j]))
                        for j in range(len(target)):
                            if abs(tmpCSIb1[i] - target[j]) == dist:
                                resCSIb1.append(target[j])
                                target[j] = np.inf
                    target = list(range(keyLen))
                    target = target - np.mean(target)
                    target = (target - np.min(target)) / (np.max(target) - np.min(target))
                    ...
                    for i in range(keyLen):
                        ...
                        dist = np.inf
                        for j in range(len(target)):
                            dist = min(dist, abs(tmpCSIe1[i] - target[j]))
                        for j in range(len(target)):
                            if abs(tmpCSIe1[i] - target[j]) == dist:
                                resCSIe1.append(target[j])
                                target[j] = np.inf
                    ...
                    # resCSIa1 += np.random.normal(0, 1, keyLen)
                    randomMatrix = np.linalg.lstsq(np.array(tmpCSIa1).reshape(1, -1), np.array(resCSIa1).reshape(1, -1), rcond=None)[0]
                    randomMatrix = randomMatrix - np.mean(randomMatrix)
                    randomMatrix = (randomMatrix - np.min(randomMatrix)) / (np.max(randomMatrix) - np.min(randomMatrix))
                    # 原始 bmr 1.0 Eve corr 1.0 bmr 0.75291617
                    # np.random.seed(staInd)
                    # bmr 0.993504736 Eve corr 0.18831390738210432 bmr 0.499674126
                    # randomMatrix += np.random.normal(0, 1, (keyLen, keyLen))
                    # bmr 1.0 Eve corr 0.45087686361020474 bmr 0.519209544
                    # randomMatrix += np.random.normal(np.mean(randomMatrix), np.std(randomMatrix), (keyLen, keyLen))
                    # randomMatrix += np.random.uniform(0, 1, (keyLen, keyLen))
                    # randomMatrix += np.random.uniform(0, np.std(CSIa1Orig) * 4, size=(keyLen, keyLen))
                    # randomMatrix = randomMatrix - np.mean(randomMatrix)
                    ...
                    # # 无target[j] = np.inf 0.981677143 0.505817841 0.007647880172632645
                    # # 有target[j] = np.inf 0.994303497 0.496808903 0.008555579443286696
                    # randomMatrix = np.linalg.lstsq(np.array(tmpCSIb1).reshape(1, -1), np.array(resCSIb1).reshape(1, -1), rcond=None)[0]
                    # randomMatrix = randomMatrix - np.mean(randomMatrix)
                    # randomMatrix = (randomMatrix - np.min(randomMatrix)) / (np.max(randomMatrix) - np.min(randomMatrix))
                    # tmpCSIb1 = np.matmul(tmpCSIb1, randomMatrix)
                    #
                    # randomMatrix = np.linalg.lstsq(np.array(tmpCSIe1).reshape(1, -1), np.array(resCSIe1).reshape(1, -1), rcond=None)[0]
                    # randomMatrix = randomMatrix - np.mean(randomMatrix)
                    # randomMatrix = (randomMatrix - np.min(randomMatrix)) / (np.max(randomMatrix) - np.min(randomMatrix))
                    # tmpCSIe1 = np.matmul(tmpCSIe1, randomMatrix)
                ...
                if withOptSortMatrix:
                    ...
                    # "l-bfgs-b"和"slsqp"    without gray code
                    # "l-bfgs-b"    1.0 1.0 0.514814815 0.514814815  /  0.004198288 0.005465013 0.000698  /  2.034750613
                    # Eve 0.803856914 0.578600823
                    # "slsqp"       0.999599479 0.999176955 0.513734568 0.513528807  /  0.004402396 0.005435518 0.000880634  /  3.103647944
                    # Eve 0.763692801 0.500411523
                ...
                if withOptMatrix:
                    ...
                    Ka += np.random.normal(0, 1, (keyLen, keyLen))
                    tmpCSIa1 = np.matmul(tmpCSIa1, Ka)
                    tmpCSIb1 = np.matmul(tmpCSIb1, Ka)
                    tmpCSIe1 = np.matmul(tmpCSIe1, Ka)
                ...
                if segment_option == "opt_pair":
                    ...
                    # if sum(segment_method_ori) != keyLen and runBackCounts2 < 6:
                    #     print("分段总数变少", "回退" + str(runBackCounts2) + "次")
                    #     isRunBack = True
                    #     staInd -= keyLenLoop
                    #     continue
                    # elif sum(segment_method_ori) != keyLen and isRunBack is True:
                    #     runBackCounts2 = -1
                    #     isRunBack = False
                    # else:
                    #     runBackCounts2 = -1
