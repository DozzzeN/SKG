...
def compute_mean_dtw(data):
    mean_dtw = 0
    for i in range(len(data)):
        for j in range(i + 1, len(data)):
            mean_dtw += dtw_metric(data[i], data[j])[0]
    return mean_dtw / (len(data) * (len(data) - 1) / 2)
...
def find_sub_opt_segment_method_down(data, min_length, num_segments):
        ...
        for segment_method in segment_methods:
            for i in range(len(segment_method)):
                ...
                for j in range(min_length, segment_method[i] - min_length + 1):
                    ...
                    distance_temp = compute_mean_dtw(segment)
...
def find_sub_opt_segment_method_up(data, base_length, num_segments):
...
def find_opt_segment_method(data, min_length, num_segments):
    ...
    for segment_method in segment_methods:
        ...
        dist = compute_mean_dtw(data_seg)
...
def search_segment_method_with_metric(dataA, dataB, segments):
    # 给定分段长度，通过搜索找到分段位置
    segmentA = segment_data(dataA, len(segments), segments)
    segmentB = segment_data(dataB, len(segments), segments)
    dists = []
        ...
        for withoutSort in withoutSorts:
            ...
            for staInd in range(0, dataLenLoop, keyLenLoop):
                ...
                if segment_option == "opt":
                    ...
                    if compute_mean_dtw(tmpCSIa1IndReshape) < compute_mean_dtw(tmpCSIa1EvenSegment):
                        print("staInd", staInd, "最优分段最大差距", compute_mean_dtw(tmpCSIa1IndReshape),
                              "平均分段最大差距", compute_mean_dtw(tmpCSIa1EvenSegment))
                        badSegments += 1
                    segmentMaxDist.append(compute_mean_dtw(tmpCSIa1IndReshape))
                    evenMaxDist.append(compute_mean_dtw(tmpCSIa1EvenSegment))
                elif segment_option == "sub_down":
                    start_time = time.time_ns()
                    segment_method = find_sub_opt_segment_method_down(tmpCSIa1Ind, 3, int(keyLen / segLen))
                    if len(segment_method) != int(keyLen / segLen):
                        print("未找到次优解")
                        continue
                    ...
                    tmpCSIa1EvenSegment = np.array(tmpCSIa1Ind).reshape(int(keyLen / segLen), segLen)
                    if compute_mean_dtw(tmpCSIa1IndReshape) < compute_mean_dtw(tmpCSIa1EvenSegment):
                        print("staInd", staInd, "次优分段最大差距", compute_mean_dtw(tmpCSIa1IndReshape),
                              "平均分段最大差距", compute_mean_dtw(tmpCSIa1EvenSegment))
                        badSegments += 1
                    segmentMaxDist.append(compute_mean_dtw(tmpCSIa1IndReshape))
                    evenMaxDist.append(compute_mean_dtw(tmpCSIa1EvenSegment))
                    ...
                    start_time = time.time_ns()
                    segment_method = find_sub_opt_segment_method_up(tmpCSIa1Ind, 2, int(keyLen / segLen))
                    if len(segment_method) != int(keyLen / segLen):
                        print("未找到次优解")
                        continue
                    print("staInd", staInd, "segment_method", segment_method)
                    ...
                    # 次优分段（是否固定分段个数）
                    # segment_method_ori = find_sub_opt_segment_method_down(tmpCSIa1Ind, 3, int(keyLen / segLen))
                    # segment_method_ori1 = find_sub_opt_segment_method_down(tmpCSIa1Ind, 3, int(keyLen / segLen))
                    # segment_method_ori2 = find_sub_opt_segment_method_down(tmpCSIa1Ind, 3, int(keyLen / segLen) + 1)
                    ...
                    # 合并分段
                    segment_method_ori = find_sub_opt_segment_method_up(tmpCSIa1Ind, 2, int(keyLen / segLen))
                ...
                if segment_option == "snd":
                    ...
                    # 根据子分段构成一个覆盖总分段长度的组合
                    # segment_method_A = find_all_cover_intervals_iter(segments_A, (0, keyLen), 3, 5)
                    # segment_method_B = find_all_cover_intervals_iter(segments_B, (0, keyLen), 3, 5)
            ...
            if len(segment_time) != 0:
                ...
                print(np.round(np.mean(segment_time), 9), np.round(np.mean(search_time), 9), np.round(np.mean(final_search_time), 9))
            else:
