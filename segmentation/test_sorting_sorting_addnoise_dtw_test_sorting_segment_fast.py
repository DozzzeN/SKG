...
from segmentation.methods import find_opt_segment_method, segment_sequence, compute_min_dtw, \
    ...
    compute_min_euclidean, find_best_matching_pair_threshold, find_best_matching_pair_threshold_filter, \
            ...
            while staInd < dataLenLoop:
                ...
                if segment_option == "find_search":
                    ...
                    if isCorrect:
                        ...
                        # correction
                        indices_a_match, shuffle_methods_a = find_best_matching_pair(tmpCSIa1IndReshape, tmpCSIa1BckReshape)
                        indices_a_match = [index[1] for index in indices_a_match]
                        a_list_number = list(indices_a_match)
                        key_hash = hash(tuple(a_list_number))
                        if key_hash != hash(tuple(b_list_number)):
                            # matching
                            indices_b_match, shuffle_methods_b = find_best_matching_pair(tmpCSIa1IndReshape, tmpCSIb1IndReshape)
                            indices_b_match = [index[1] for index in indices_b_match]
                            b_list_number = list(indices_b_match)
                            if key_hash != hash(tuple(b_list_number)):
                                # filter
