...
def levelNoiseMetricSortPerm(data, length, noise, metric):
    ...
    for i in range(0, len(metrics) - 2):
        # average
        average_metrics = adjacentMean(np.argsort(metrics[i + 1]))
        p = pearson_metric(np.argsort(metrics[i]), average_metrics)
        p = 0 if math.isnan(p) else p
        print("第" + str(i) + "-" + str(i + 1) + "层", np.argsort(metrics[i]), np.argsort(metrics[i + 1]),
              average_metrics, p)
        # p = abs(p)
        corr.append(p)

        # rescale
        # rescale_metrics = rescale(np.argsort(metrics[i]), np.argsort(metrics[i + 1]))
        # corr.append(pearson_metric(np.argsort(metrics[i]), rescale_metrics))
        # print("第" + str(i) + "-" + str(i + 1) + "层", np.argsort(metrics[i]), np.argsort(metrics[i + 1]),
        #       rescale_metrics)
        # t1 = np.random.permutation(range(len(metrics[i])))
        # t2 = np.random.permutation(range(len(metrics[i + 1])))
        # rescale_t2 = rescale(t1, t2)
        # random_corr.append(pearson_metric(t1, rescale_t2))

        # expand_rescale
        # rescale_metrics = expand_rescale(np.argsort(metrics[i]))
        # corr.append(pearson_metric(rescale_metrics, np.argsort(metrics[i + 1])))
        # print("第" + str(i) + "-" + str(i + 1) + "层", np.argsort(metrics[i]), np.argsort(metrics[i + 1]),
        #       rescale_metrics)
        # t1 = np.random.permutation(range(len(metrics[i])))
        # t2 = np.random.permutation(range(len(metrics[i + 1])))
        # rescale_t1 = expand_rescale(t1)
        # random_corr.append(pearson_metric(rescale_t1, t2))

        # adj_rescale
        # rescale_metrics = adj_rescale(np.argsort(metrics[i + 1]))
        # corr.append(pearson_metric(np.argsort(metrics[i]), rescale_metrics))
        # print("第" + str(i) + "-" + str(i + 1) + "层", np.argsort(metrics[i]), np.argsort(metrics[i + 1]),
        #       rescale_metrics)
        # t1 = np.random.permutation(range(len(metrics[i])))
        # t2 = np.random.permutation(range(len(metrics[i + 1])))
        # rescale_t2 = adj_rescale(t2)
        # random_corr.append(pearson_metric(t1, rescale_t2))
