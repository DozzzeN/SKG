...
for staInd in range(0, len(CSIa1Orig), keyLen):
    ...
    a_list_number, a_point, a_data = list(listToHilbertCurveIndexWithFixedParams(projCSIa1XY, hilbert_p, grid_size, dimension))
    b_list_number, b_point, b_data = list(listToHilbertCurveIndexWithFixedParams(projCSIb1XY, hilbert_p, grid_size, dimension))
    # min_of_a = [findMinX(projCSIe1XY), findMinY(projCSIe1XY)]
    e_list_number, e_point, _ = list(listToHilbertCurveIndexWithFixedParams(projCSIe1XY, hilbert_p, grid_size, dimension))
    # min_of_a = [findMinX(projCSIn1XY), findMinY(projCSIn1XY)]
    n_list_number, n_point, _ = list(listToHilbertCurveIndexWithFixedParams(projCSIn1XY, hilbert_p, grid_size, dimension))
