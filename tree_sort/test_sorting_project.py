...
def normal2uniform(data, shuffle):
    data_back = data.copy()
...
for staInd in range(0, len(CSIa1Orig), keyLen):
    ...
    if rawOp == "fft":
        ...
        sortCSIa1 = normal2uniform(tmpCSIa1, shuffle)
        sortCSIb1 = normal2uniform(tmpCSIb1, shuffle)
        sortCSIe1 = normal2uniform(tmpCSIe1, shuffle)
        sortNoise = normal2uniform(tmpNoise, shuffle)
    elif rawOp == "coxbox":
        ...
        sortCSIe1 = scipy.stats.boxcox(tmpCSIe1 - np.min(tmpCSIe1) + 0.1)[0]
        sortNoise = scipy.stats.boxcox(tmpNoise - np.min(tmpNoise) + 0.1)[0]
        sortCSIa1 = normal2uniform(sortCSIa1, shuffle)
        sortCSIb1 = normal2uniform(sortCSIb1, shuffle)
        sortCSIe1 = normal2uniform(sortCSIe1, shuffle)
        sortNoise = normal2uniform(sortNoise, shuffle)
    ...
    sortCSIa1 = preprocessing.MinMaxScaler().fit_transform(np.array(sortCSIa1).reshape(-1, 1)).reshape(1, -1).tolist()[0]
    sortCSIb1 = preprocessing.MinMaxScaler().fit_transform(np.array(sortCSIb1).reshape(-1, 1)).reshape(1, -1).tolist()[0]
    sortCSIe1 = preprocessing.MinMaxScaler().fit_transform(np.array(sortCSIe1).reshape(-1, 1)).reshape(1, -1).tolist()[0]
    sortNoise = preprocessing.MinMaxScaler().fit_transform(np.array(sortNoise).reshape(-1, 1)).reshape(1, -1).tolist()[0]
