...
sys.path.append("..")
from alignment import alignFloat, genAlign3, genLongestContinuous2, genAlign2
...
for staInd in range(0, times * intvl + 1, intvl):
    ...
    print("sortCSIi1", len(sortCSIi1), list(sortCSIi1))
    ...
    for i in range(max(len(ruleStr1), len(ruleStr2))):
        ...
        if len(ruleStr1[i]) != len(ruleStr2[i]):
            print("\033[0;30;42m", i, list(sortCSIi1[i * step: (i + 1) * step]), "\033[0m")
        ...
        for j in range(max(len(ruleStr1[i]), len(ruleStr2[i]))):
            ...
            if ruleStr1[i][j] != ruleStr2[i][j]:
                print("\033[0;30;41m", i, list(sortCSIi1[i * step: (i + 1) * step]), "\033[0m")
