...
for staInd in range(0, 10 * intvl + 1, intvl):
    ...
    for i in range(len(CSIa1Back)):
        ...
        for j in range(len(CSIa1Back)):
            ...
            aa_d = average_hd(CSIa1Back[i], sortCSIa1[j])
            ab_d = average_hd(CSIb1Back[i], sortCSIb1[j])
            ae_d = average_hd(CSIe1Back[i], sortCSIe1[j])
            an_d = average_hd(CSIn1Back[i], sortNoise[j])
            if aa_d < aa_hd:
                aa_hd = aa_d
                aa_index = j
            if ab_d < ab_hd:
