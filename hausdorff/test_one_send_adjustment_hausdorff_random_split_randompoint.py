...
# 导入随机路点
def genRandomPoint(length, range):
    x_range = np.array([0, range])
    y_range = np.array([0, range])
    model = RandomWayPoint(steps=length, x_range=x_range, y_range=y_range)
    data1 = model.generate_trace(start_coor=[1, 1])
    data1 = data1.flatten()
    noise = np.array(np.random.normal(loc=0, scale=1, size=length * 2))
    data2 = data1 + noise
    return data1, data2


CSIa1Orig, CSIb1Orig = genRandomPoint(6745, 50)
