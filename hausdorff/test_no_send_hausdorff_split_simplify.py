...
def rotate(list):
    ...
    for i in range(len(list)):
        ...
        # 旋转中心：多边形的质心
        # center = Polygon(list[i]).centroid.coords[0]
        random.seed(0)
        center = (random.uniform(0, 1), random.uniform(0, 1))
        for j in range(len(list[i])):
            x = list[i][j][0]
            y = list[i][j][1]
            list[i][j][0] = (x - center[0]) * math.cos(angle) - (y - center[1]) * math.sin(angle)
            list[i][j][1] = (x - center[0]) * math.sin(angle) + (y - center[1]) * math.cos(angle)
            minx = min(minx, list[i][j][0])
            miny = min(miny, list[i][j][1])
...
for f in fileName:
    ...
    for staInd in range(0, dataLenLoop, keyLenLoop):
        ...
        # 绘图
        # xa, ya = zip(*CSIa1Back[0])
        # xb, yb = zip(*CSIb1Back[0])
        # plt.figure()
