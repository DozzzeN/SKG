...
for staInd in range(0, 10 * intvl + 1, intvl):
    ...
    # plt.figure()
    # plt.plot(tmpCSIa1, color="red", linewidth=.5, label="a")
    # plt.plot(tmpCSIb1, color="blue", linewidth=.5, label="b")
    # plt.plot(tmpCSIe1, color="green", linewidth=.5, label="e")
    # plt.legend(loc='upper left')
    # plt.show()
    ...
    # tmpCSIa1 = np.cos(tmpCSIa1) * np.cos(tmpNoise)
    # tmpCSIb1 = np.cos(tmpCSIb1) * np.cos(tmpNoise)
    # tmpCSIe1 = np.cos(tmpCSIe1) * np.cos(tmpNoise)

    corCSIa1 = pearsonr(tmpCSIa1, tmpNoise)[0]
    corCSIb1 = pearsonr(tmpCSIb1, tmpNoise)[0]
    corCSIe1 = pearsonr(tmpCSIe1, tmpNoise)[0]

    print("corCSIa1:", corCSIa1, "corCSIb1:", corCSIb1, "corCSIe1:", corCSIe1)
    ...
    # plt.figure()
    # plt.plot(sortCSIa1[0:64], color="red", linewidth=1.5, label="a")
    # plt.plot(sortCSIb1[0:64], color="blue", linewidth=1.5, label="b")
    # plt.plot(sortCSIe1[0:64], color="green", linewidth=1.5, label="e")
    # plt.legend(loc='upper left')
    # plt.show()
...
# 加噪声
# a-b all 352 / 352 = 1.0
# a-e all 172 / 352 = 0.48863636363636365
# a-n all 7 / 352 = 0.019886363636363636

# 不加噪声
# a-b all 348 / 352 = 0.9886363636363636
# a-e all 80 / 352 = 0.22727272727272727
# a-n all 10 / 352 = 0.028409090909090908
